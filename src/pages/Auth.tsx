...
export default function Auth() {
  ...
  // Capturar o redirect da URL ou do state (para convites e ProtectedRoute)
  const redirectTo = searchParams.get("redirect") || (location.state as any)?.from || null;

  // ── If AuthContext is still bootstrapping, show spinner ──
  ...
  // ── Helpers for brute-force protection ──
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // Redirect handled by AuthContext state change → re-render → Navigate
        const destination = redirectTo || "/";
