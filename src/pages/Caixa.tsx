...
export default function Caixa() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch total bookmaker balance - agregar por moeda separando Broker
      ...
      setSaldosBrokerPorMoeda(
        Object.entries(saldosBrokerPorMoedaMap)
          .filter(([_, saldo]) => saldo !== 0)
          .map(([moeda, saldo]) => ({ moeda, saldo }))
      );
      
      // Legacy: manter compatibilidade com saldoBookmakers (BRL não-Broker para CaixaTabsContainer)
  ...
  // Payload estável para o dialog de confirmação de saque.
  // Recriar este objeto a cada render reinicializava o formulário do dialog,
  // sobrescrevendo o valor digitado pelo operador com a estimativa do saque.
  const saqueConfirmacaoPayload = useMemo(() => {
    if (!saqueParaConfirmar) return null;
  ...
  }, [saqueParaConfirmar, walletsDetalhes, bookmakers, parceiros, contasBancarias]);
      ...
      <div className="flex-1 min-h-0 overflow-y-auto bg-[var(--bg-page)] custom-scrollbar">
        <div className="px-6 py-4 space-y-[10px]">
          ...
          <PosicaoCapital
            ...
            saldosBroker={saldosBrokerPorMoeda}
