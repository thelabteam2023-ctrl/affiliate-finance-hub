...
export function ProjetoApostasTab({ projetoId, onDataChange, refreshTrigger, formatCurrency: formatCurrencyProp, actionsSlot }: ProjetoApostasTabProps) {
  ...
  const fetchApostas = async () => {
    try {
      ...
      const selectFields = `
          id, created_at, data_aposta, esporte, evento, mercado, selecao, odd, stake, stake_real, stake_freebet, stake_total, estrategia,
        ...
        const pernasData = await fetchChunkedIn(
          (idsChunk) =>
            supabase
              .from("apostas_pernas")
              .select(`
                aposta_id, id, bookmaker_id, odd, stake, stake_real, stake_freebet, moeda, selecao, selecao_livre, ordem, tipo, comissao, resultado, lucro_prejuizo, fonte_saldo,
  ...
  const apostasUnificadas = useMemo(() => {
    ...
    return lista.filter(u => {
      ...
      const selecoes = ('selecoes' in d ? (d as any).selecoes : null);
      const matchesPernas = Array.isArray(pernas) && pernas.some((p: any) => (p?.bookmaker_nome || '').toLowerCase().includes(term) || (p?.selecao || '').toLowerCase().includes(term));
      const matchesSelecoes = Array.isArray(selecoes) && selecoes.some((s: any) => (s?.descricao || '').toLowerCase().includes(term));
