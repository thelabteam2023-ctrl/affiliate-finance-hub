...
export function ProjetoValueBetTab({ 
  ...
  const fetchApostas = async () => {
    try {
      ...
      // Query adicional: LIQUIDADAS recém-atualizadas no range (cobre bets com data_aposta fora do range, ex.: futuro)
  ...
  const handleDuplicateSimples = useCallback((apostaId: string) => {
    const url = `/janela/aposta/novo?projetoId=${encodeURIComponent(projetoId)}&tab=valuebet&estrategia=VALUEBET&duplicateFrom=${apostaId}`;
  ...
  const openEditDialog = useCallback((a: Aposta) => {
    // Apostas registradas pelo novo formulário "Nova Entrada" abrem inline
    // no MESMO formulário em modo edição.
