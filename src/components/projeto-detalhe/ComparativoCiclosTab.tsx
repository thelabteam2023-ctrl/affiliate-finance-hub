    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Visão geral da performance nos ciclos</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-3">
            {/* Melhor Ciclo (Lucro) */}
            {analises?.melhorLucro && (
              <div className="p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                <div className="flex items-center gap-2 mb-2">
                  <Award className="h-4 w-4 text-emerald-500" />
                  <span className="text-sm font-medium text-emerald-500">Melhor Lucro</span>
                </div>
                <p className="text-xl font-bold">Ciclo {analises.melhorLucro.numero_ciclo}</p>
                <p className="text-sm text-muted-foreground">{formatCurrency(analises.melhorLucro.lucro)}</p>
              </div>
            )}
            ...
            {analises?.melhorRoi && (
              <div className="p-4 rounded-lg bg-blue-500/10 border border-blue-500/20">
                ...
                <p className="text-xl font-bold">Ciclo {analises.melhorRoi.numero_ciclo}</p>
                <p className="text-sm text-muted-foreground">{analises.melhorRoi.roi.toFixed(2)}%</p>
              </div>
            )}

            {/* Melhor Ciclo (Eficiência) */}
            {analises?.melhorEficiencia && (
              <div className="p-4 rounded-lg bg-purple-500/10 border border-purple-500/20">
                <div className="flex items-center gap-2 mb-2">
                  <Zap className="h-4 w-4 text-purple-500" />
                  <span className="text-sm font-medium text-purple-500">Mais Eficiente</span>
                </div>
                <p className="text-xl font-bold">Ciclo {analises.melhorEficiencia.numero_ciclo}</p>
                <p className="text-sm text-muted-foreground">{formatCurrency(analises.melhorEficiencia.lucroPoAposta)}/aposta</p>
              </div>
            )}
          </div>
          ...
          {analises?.melhorMes && (
            <div className="p-4 rounded-lg bg-muted/50">
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium capitalize">Melhor período: {analises.melhorMes.mes}</span>
              </div>
              <p className="text-sm text-muted-foreground mt-1">
                {formatCurrency(analises.melhorMes.lucro)} de lucro total | {analises.melhorMes.apostas} apostas | {formatCurrency(analises.melhorMes.lucroMedio)} por aposta
              </p>
            </div>
          )}
          ...
          <div className="flex items-center gap-3 p-4 rounded-lg bg-muted/30">
            {analises?.tendencia === "crescimento" && (
              <>
                <TrendingUp className="h-5 w-5 text-emerald-500" />
                <div>
                  <p className="font-medium text-emerald-500">Tendência de Crescimento</p>
                  <p className="text-sm text-muted-foreground">Os últimos ciclos mostram evolução positiva</p>
                </div>
              </>
            )}
            {analises?.tendencia === "queda" && (
              <>
                <TrendingDown className="h-5 w-5 text-red-500" />
                <div>
                  <p className="font-medium text-red-500">Tendência de Queda</p>
                  <p className="text-sm text-muted-foreground">Os últimos ciclos mostram retração nos resultados</p>
                </div>
              </>
            )}
            {analises?.tendencia === "estabilidade" && (
              <>
                <Minus className="h-5 w-5 text-muted-foreground" />
                <div>
                  <p className="font-medium">Tendência Estável</p>
                  <p className="text-sm text-muted-foreground">Resultados mantendo-se consistentes</p>
                </div>
              </>
            )}
          </div>
