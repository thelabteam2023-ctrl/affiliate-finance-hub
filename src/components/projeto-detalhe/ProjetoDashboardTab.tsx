...
export function ProjetoDashboardTab({ projetoId }: ProjetoDashboardTabProps) {
  ...
  const { formatCurrency, formatChartAxis, convertToConsolidation, convertToConsolidationOficial, moedaConsolidacao, cotacaoAtual } = useProjetoCurrency(projetoId);
  ...
  const { daily: calendarDaily, resumo: calendarResumo } = useCalendarApostasRpc({ projetoId, cotacaoUSD: cotacaoAtual });
