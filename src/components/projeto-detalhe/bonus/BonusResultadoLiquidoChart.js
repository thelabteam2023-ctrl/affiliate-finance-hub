...
import { Receipt, AreaChart as AreaChartIcon, Activity, Filter, X, Calendar, Info, Target } from "lucide-react";
...
import { ResponsiveContainer, AreaChart, Area, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ReferenceLine, Cell, } from "recharts";
import { format, startOfDay, startOfYear, endOfDay } from "date-fns";
import { extractLocalDateKey, extractCivilDateKey } from "@/utils/dateUtils";
import { ptBR } from "date-fns/locale";
import { CalendarioLucros } from "../CalendarioLucros";
import { resolveCalendarInitialMonth } from "@/utils/calendarInitialMonth";
export function BonusResultadoLiquidoChart({ bonuses, bonusBets, pernasMap = {}, ajustesPostLimitacao = [], perdasCancelamento = [], formatCurrency, convertToConsolidation, moedaConsolidacao, isSingleDayPeriod = false, dateRange, potencialBruto = 0, }) {
    const [chartMode, setChartMode] = useState("resultado");
    ...
    const bookmakerStats = useMemo(() => {
        ...
        bonuses
            ...
            if (internalDateRange) {
                ...
                if (bonusDate < startOfDay(internalDateRange.start) || bonusDate > internalDateRange.end)
                    return;
            }
    ...
    const chartData = useMemo(() => {
        ...
        filteredBonuses
            ...
            if (effectiveDateRange) {
                ...
                if (bonusDate < startOfDay(effectiveDateRange.start) || bonusDate > effectiveDateRange.end)
                    return;
            }
            const rawAmount = b.bonus_amount || 0;
            const hasSnapshot = typeof b.valor_consolidado_snapshot === 'number' && b.valor_consolidado_snapshot > 0;
        ...
        bonusBets.forEach(bet => {
            ...
            if (effectiveDateRange) {
                ...
                if (betDate < startOfDay(effectiveDateRange.start) || betDate > effectiveDateRange.end)
                    return;
            }
            ...
            if (selectedBookmaker && bet.bonus_id) {
                ...
                if (relatedBonus && relatedBonus.bookmaker_id !== selectedBookmaker)
                    return;
            }
            // CORREÇÃO: Usar extractLocalDateKey em vez de split("T")[0]
            const date = extractLocalDateKey(bet.data_aposta);
            const pl = getConsolidatedLucroDirect({ lucro_prejuizo: bet.lucro_prejuizo, moeda_operacao: bet.moeda_operacao, pl_consolidado: bet.pl_consolidado, consolidation_currency: bet.consolidation_currency, is_multicurrency: bet.is_multicurrency }, pernasMap[bet.id], convertToConsolidation, moedaConsolidacao);
        ...
        ajustesPostLimitacao.forEach(ajuste => {
            ...
            if (selectedBookmaker && ajuste.bookmaker_id !== selectedBookmaker)
                return;
            ...
            if (effectiveDateRange) {
                ...
                if (ajusteDate < startOfDay(effectiveDateRange.start) || ajusteDate > effectiveDateRange.end)
                    return;
            }
        ...
        perdasCancelamento.forEach(perda => {
            if (selectedBookmaker && perda.bookmaker_id !== selectedBookmaker)
                return;
            const date = extractCivilDateKey(perda.data_operacional);
            if (effectiveDateRange) {
                const perdaDate = new Date(date + "T12:00:00");
                if (perdaDate < startOfDay(effectiveDateRange.start) || perdaDate > effectiveDateRange.end)
                    return;
            }
            const valor = convertToConsolidation
    ...
    const benchmarkLevels = useMemo(() => {
        if (!potencialBruto || potencialBruto <= 0)
            return [];
        return [
    ...
    const calendarInitialMonth = useMemo(() => resolveCalendarInitialMonth(internalDateRange?.start, internalDateRange?.end), [internalDateRange?.start, internalDateRange?.end]);
    ...
    const getPerformanceColorClass = (percent) => {
        if (percent >= 70)
    ...
    const renderKPIsTooltipContent = () => {
        switch (chartMode) {
            ...
            <div className="space-y-1">
              ...
              {potencialBruto > 0 && eficiencia !== null && (<div className="border-t border-border/50 pt-1 mt-1 space-y-1">
                  ...
                  <div className="flex justify-between gap-4 text-xs">
                    ...
                    <span className={`font-semibold ${eficiencia >= 70 ? "text-emerald-500" :
                            eficiencia >= 50 ? "text-warning" :
                                "text-destructive"}`}>{eficiencia.toFixed(1)}%</span>
                  </div>
                </div>)}
            </div>
          </div>);
    ...
    const renderBonusJuiceFilters = () => {
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="outline" size="sm" className="h-6 px-2 text-xs gap-1">
              ...
              {selectedBookmakerName ? (<span className="max-w-[100px] truncate">{selectedBookmakerName}</span>) : ("Todas as casas")}
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="w-64">
            <DropdownMenuLabel className="text-xs">Filtrar por casa</DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => setSelectedBookmaker(null)} className="text-xs">
            ...
            <ScrollArea className="h-[200px]">
              {bookmakerStats.map((stat) => (<DropdownMenuItem key={stat.bookmaker_id} onClick={() => setSelectedBookmaker(stat.bookmaker_id)} className="text-xs flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {selectedBookmaker === stat.bookmaker_id && (<Badge variant="secondary" className="text-[10px] h-4">Ativo</Badge>)}
                  </div>
                </DropdownMenuItem>))}
            </ScrollArea>
          </DropdownMenuContent>
        ...
        {selectedBookmaker && (<Button variant="ghost" size="sm" className="h-6 px-1.5 text-xs text-muted-foreground hover:text-foreground" onClick={() => setSelectedBookmaker(null)}>
            <X className="h-3 w-3"/>
          </Button>)}
      </>);
    };
    ...
    const renderCustomDot = (props) => {
        ...
        if (!payload?.isLastPoint || !showBenchmark)
            return null;
    ...
    const renderChart = () => {
        ...
        switch (chartMode) {
            case "resultado":
                return (<AreaChart data={chartData} margin={{ top: 10, right: showBenchmark ? 120 : 10, left: 0, bottom: 0 }}>
            <defs>
            ...
            <XAxis dataKey="label" stroke="hsl(var(--muted-foreground))" fontSize={11} tickLine={false} axisLine={false} interval={useMonthlyTicks ? 0 : undefined} tick={({ x, y, payload, index }) => {
                        if (useMonthlyTicks && monthlyTickIndices) {
                            // Only render ticks at month boundaries
                            if (!monthlyTickIndices.has(index))
                                return null;
                            ...
                            return (<text x={x} y={y + 10} textAnchor="middle" fill="hsl(var(--muted-foreground))" fontSize={11} fontWeight={500}>
                      {label}
                    </text>);
                        }
                        ...
                        if (!payload.value)
                            return null;
                        return (<text x={x} y={y + 10} textAnchor="middle" fill="hsl(var(--muted-foreground))" fontSize={11}>
                    {payload.value}
                  </text>);
                    }}/>
            <YAxis stroke="hsl(var(--muted-foreground))" fontSize={11} tickLine={false} axisLine={false} width={60} domain={showBenchmark && benchmarkLevels.length > 0
                        ? [
                            (dataMin) => Math.min(dataMin, 0),
                            (dataMax) => Math.max(dataMax, ...benchmarkLevels.map(l => l.value)) * 1.05,
                        ]
                        : undefined} tickFormatter={(value) => {
            ...
            {/* Benchmark lines */}
            {showBenchmark && benchmarkLevels.map((level) => (<ReferenceLine key={`benchmark-${level.percent}`} y={level.value} stroke={level.color} strokeDasharray="8 4" strokeOpacity={0.6} strokeWidth={1.5} label={{
            ...
            <Area type="monotone" dataKey="acumulado" stroke={isPositivo ? colorPositivo : colorNegativo} strokeWidth={2} fill="url(#bonusGradient)" dot={renderCustomDot} activeDot={{ r: 4, strokeWidth: 2 }}/>
          </AreaChart>);
            case "bonus_juice":
                // Gráfico de barras agrupadas: Bônus e Juice lado a lado
                return (<BarChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }} barCategoryGap="20%" barGap={2}>
            <defs>
            ...
            <XAxis dataKey="label" stroke="hsl(var(--muted-foreground))" fontSize={10} tickLine={false} axisLine={false} interval="preserveStartEnd" tickMargin={8}/>
            <YAxis stroke="hsl(var(--muted-foreground))" fontSize={10} tickLine={false} axisLine={false} width={55} tickCount={5} tickFormatter={(value) => {
                        if (Math.abs(value) >= 1000)
                            return `${(value / 1000).toFixed(1)}k`;
                        return value.toFixed(0);
                    }}/>
            ...
            <Bar dataKey="bonus_creditado" name="Bônus" fill="url(#bonusBarGradient)" radius={[4, 4, 0, 0]} maxBarSize={24}/>
            {/* Barra de Juice (verde/vermelho) */}
            <Bar dataKey="juice" name="Juice" radius={[4, 4, 0, 0]} maxBarSize={24}>
              {chartData.map((entry, index) => (<Cell key={`cell-juice-${index}`} fill={entry.juice >= 0 ? "url(#juicePositiveGradient)" : "url(#juiceNegativeGradient)"}/>))}
            </Bar>
          </BarChart>);
        }
    };
    return (<Card>
      <CardHeader className="pb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
              ...
              <PopoverContent className="w-auto p-0" align="end">
                <CalendarioLucros apostas={calendarApostas} titulo="Resultado Líquido de Bônus" accentColor="amber" compact formatCurrency={formatCurrency} initialMonth={calendarInitialMonth}/>
              </PopoverContent>
            </Popover>
            
            {/* Toggle de modos */}
            <ToggleGroup type="single" value={chartMode} onValueChange={(value) => value && setChartMode(value)} className="justify-start">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-2">
          ...
          {chartMode === "resultado" && potencialBruto > 0 && (<div className="flex items-center gap-1.5 ml-auto">
              {eficiencia !== null && showBenchmark && (<Badge variant="outline" className={`text-[10px] h-5 ${eficiencia >= 70 ? "border-emerald-500/50 text-emerald-500 bg-emerald-500/10" :
                ...
                <UITooltip>
                  <TooltipTrigger asChild>
                    <label className="flex items-center gap-1.5 cursor-pointer">
                      ...
                      <Switch checked={showBenchmark} onCheckedChange={setShowBenchmark} className="h-4 w-7 data-[state=checked]:bg-chart-4"/>
                    </label>
                  </TooltipTrigger>
      ...
      <CardContent className="pt-2">
        ...
        {showBenchmark && chartMode === "resultado" && benchmarkLevels.length > 0 && (<div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1 mt-3 pt-2 border-t border-border/30">
            ...
            {benchmarkLevels.map((level) => (<div key={level.percent} className="flex items-center gap-1.5">
...
function ResultadoTooltip({ active, payload, label, formatCurrency, showBenchmark, benchmarkLevels, kpis }) {
    ...
    return (<div className="bg-card border border-border rounded-lg p-3 shadow-lg max-w-[280px]">
      <p className="text-xs text-muted-foreground mb-2">Data: {label}</p>
      <div className="space-y-1">
        {data.bonus_creditado > 0 && (<div className="flex justify-between gap-4 text-xs">
      ...
      {isLast && (<div className="border-t border-border/50 mt-2 pt-2 space-y-1.5">
          ...
          {benchmarkLevels.map((level) => {
                ...
                return (<div key={level.percent} className="flex justify-between gap-2 text-xs">
...
function BonusJuiceTooltip({ active, payload, label, formatCurrency }) {
    if (!active || !payload || payload.length === 0)
        return null;
    const data = payload[0]?.payload;
    if (!data)
    ...
    return (<div className="bg-popover/95 backdrop-blur-sm border border-border/50 rounded-lg p-3 shadow-xl min-w-[180px]">
