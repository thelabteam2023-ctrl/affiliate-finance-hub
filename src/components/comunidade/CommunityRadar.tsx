...
export function CommunityRadar() {
  ...
  const fetchRadarData = async () => {
    try {
      ...
      // Calculate engagement score (weighted: recent activity = 2x)
      bookmakerMap.forEach((item) => {
        item.engagement_score = 
          (item.topicos_recentes * 3) + // Recent topics worth 3 points
          (item.comentarios_recentes * 2) + // Recent comments worth 2 points
          (item.total_topicos * 1) + // Total topics worth 1 point
          (item.total_comentarios * 0.5); // Total comments worth 0.5 points
      });

      // Convert to array, filter out zero engagement, sort by score
    ...
    <Card>
      <CardHeader className="pb-3">
        ...
        <p className="text-xs text-muted-foreground">
          Casas mais discutidas (últimos 30 dias)
        </p>
      </CardHeader>
      <CardContent>
