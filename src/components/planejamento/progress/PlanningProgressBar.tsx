...
export function PlanningProgressBar({
  ...
  const { format, symbol } = useFormatProjetoCurrency(projetoId);

  const metrics = useMemo(() => {
    return calculatePlanningMetrics(campanhas, year, month, convertToConsolidation);
  }, [campanhas, year, month, convertToConsolidation]);
  ...
  return (
    <div className={cn("space-y-3 p-4 border rounded-xl bg-card/50 shadow-sm", className)}>
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-2">
        <div className="space-y-0.5">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="text-xl font-black tracking-tight">{format(metrics.completed)}</span>
            <span className="text-xs text-muted-foreground font-medium">
              executados de {format(metrics.totalPlanned)}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col items-start sm:items-end gap-1">
          <div className="flex items-center gap-2">
            ...
            <span className={cn("text-xs font-bold", gapColor)}>
              {format(Math.abs(metrics.gap))} {metrics.isAtrasado ? "abaixo" : "acima"} do esperado
            </span>
          </div>
          <p className="text-[10px] text-muted-foreground font-medium">
            Meta temporal até hoje: <span className="text-foreground">{format(metrics.expectedToday)}</span>
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-tight text-muted-foreground px-0.5">
        ...
        <span className="opacity-60">Faltam {format(metrics.totalPlanned - metrics.completed)}</span>
      </div>
    </div>
  );
}