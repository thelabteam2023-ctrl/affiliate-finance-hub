...
import { Loader2, AlertTriangle, User, Link2, KeyRound, Coins, StickyNote, Check, Copy } from "lucide-react";
...
export default function BookmakerDialog({ open, onClose, onCreated, bookmaker, defaultParceiroId, defaultBookmakerId, lockParceiro = false, lockBookmaker = false }) {
    ...
    const [moedaOperacional, setMoedaOperacional] = useState("USD");
    ...
    const checkFinancialOperations = async (bookmakerId) => {
        if (!bookmakerId) {
    ...
    const fetchBookmakerDetails = async (bookmakerIdToFetch, presetLink, preserveMoeda = false) => {
        if (!bookmakerIdToFetch)
            return;
        ...
        try {
            ...
            if (!preserveMoeda && bookmakerData.moeda_padrao) {
                setMoedaOperacional(bookmakerData.moeda_padrao);
            }
    ...
    const fetchParceiroNome = async (parceiroIdToFetch) => {
        if (!parceiroIdToFetch)
            return;
        try {
    ...
    useEffect(() => {
        ...
        if (bookmaker) {
            ...
            setLoginPassword("");
            setMoedaOperacional(bookmaker.moeda || "BRL");
    ...
    const handleBookmakerChange = (newBookmakerId) => {
        setBookmakerId(newBookmakerId);
        setSelectedBookmaker(null);
        setSelectedLink("");
        setCopiedLink(null);
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto p-0 gap-0">
        ...
        {isWaitingForData ? (<div className="flex items-center justify-center py-16">
            ...
            <div className="px-5 py-4 space-y-5">
              ...
              <FormSection icon={User} title="Vínculo">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1">
                        {lockParceiro && parceiroId ? (<div className="flex items-center gap-2.5 h-9 border rounded-md bg-muted/30 px-3">
                            <User className="h-3.5 w-3.5 text-muted-foreground shrink-0"/>
                            <span className="text-sm font-medium uppercase truncate">
                              {parceiroNome || "Carregando..."}
                            </span>
                          </div>) : (<ParceiroSelect key={open ? 'parceiro-open' : 'parceiro-closed'} value={parceiroId} onValueChange={(newParceiroId) => {
                    setParceiroId(newParceiroId);
                ...
                }} disabled={loading} includeParceiroId={bookmaker?.parceiro_id}/>)}
                      </div>
                      {parceiroId && (<Button type="button" variant="outline" size="icon" className="shrink-0 h-9 w-9 border-dashed hover:border-primary/50 hover:bg-primary/5 transition-colors" onClick={() => setShowParceiroView(true)} title="Consultar dados do parceiro">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {lockBookmaker && bookmakerId ? (<div className="flex items-center gap-2.5 h-9 border rounded-md bg-muted/30 px-3">
                        {isLoadingDetails ? (<Loader2 className="h-4 w-4 animate-spin text-muted-foreground"/>) : selectedBookmaker ? (<>
                            <BookmakerLogo logoUrl={selectedBookmaker.logo_url} alt={selectedBookmaker.nome} size="h-5 w-5" iconSize="h-3 w-3"/>
                      ...
                      </div>) : (<BookmakerSelect key={open ? `bookmaker-${parceiroId || 'none'}` : 'bookmaker-closed'} value={bookmakerId} onValueChange={handleBookmakerChange} disabled={loading} excludeVinculosDoParceiro={!bookmaker ? parceiroId : undefined}/>)}
                  </div>
                </div>

                {/* Multi-conta identifier */}
                {showInstanceField && (<div className="space-y-1.5">
                    <Label className="text-xs flex items-center gap-1.5">
                      Identificador da Conta
                      <span className="text-[10px] text-muted-foreground">(recomendado)</span>
                    </Label>
                    <Input value={instanceIdentifier} onChange={(e) => setInstanceIdentifier(e.target.value)} placeholder="Ex: Principal, Backup, #1" disabled={loading} className="h-9 text-sm" maxLength={50}/>
              ...
              {/* Loading details */}
              {isLoadingDetails && bookmakerId && (<div className="flex items-center justify-center py-6 border rounded-lg bg-muted/20">
                  <Loader2 className="h-4 w-4 animate-spin text-muted-foreground"/>
                  <span className="ml-2 text-xs text-muted-foreground">Carregando detalhes...</span>
                </div>)}

              {/* ═══ SECTION: Link de Cadastro ═══ */}
              {hasLinks && (<FormSection icon={Link2} title="Link de Cadastro">
                  <RadioGroup value={selectedLink} onValueChange={setSelectedLink}>
                    <div className="space-y-1.5">
                      {selectedBookmaker.links_json.map((link) => (<label key={link.referencia} htmlFor={`link-${link.referencia}`} className={cn("flex items-center gap-3 px-3 py-2.5 border rounded-lg cursor-pointer transition-all", selectedLink === link.referencia
                    ...
                    }} className="h-7 w-7 p-0 shrink-0" aria-label="Copiar link de cadastro">
                            {copiedLink === link.url ? (<Check className="h-3.5 w-3.5 text-green-500"/>) : (<Copy className="h-3.5 w-3.5 text-muted-foreground"/>)}
                          </Button>
                        </label>))}
                    </div>
                  </RadioGroup>
              ...
              {!isLoadingDetails && selectedBookmaker && (<FormSection icon={Coins} title="Moeda Operacional" badge={<>
                      {checkingOperations && <Loader2 className="h-3 w-3 animate-spin text-muted-foreground"/>}
                      {hasFinancialOperations && !checkingOperations && (<Badge variant="secondary" className="text-[9px] py-0 px-1.5 h-4">🔒 Bloqueada</Badge>)}
                    </>}>
                  <div className={cn("rounded-lg border p-3 space-y-2.5", hasFinancialOperations
                    ? "border-muted bg-muted/10"
                    : "border-amber-500/30 bg-amber-500/5")}>
                    {!hasFinancialOperations && (<p className="text-[11px] text-amber-500 leading-tight">
                        Campo crítico — impacta todas as operações financeiras. Não poderá ser alterado após a primeira transação confirmada.
                      </p>)}
                    <Select value={moedaOperacional} onValueChange={(val) => {
                    setMoedaOperacional(val);
                    setMoedaConfirmada(false);
                      ...
                      <SelectTrigger className={cn("h-9 text-sm", hasFinancialOperations && "bg-muted/50 cursor-not-allowed")}>
                       ...
                       <SelectContent>
                         {FIAT_CURRENCIES.map((currency) => (<SelectItem key={currency.value} value={currency.value}>
                             <div className="flex items-center gap-2">
                    ...
                    {hasFinancialOperations ? (<p className="text-[11px] text-destructive flex items-center gap-1">
                        <AlertTriangle className="h-3 w-3 shrink-0"/>
                        Existem transações confirmadas — moeda bloqueada.
                      </p>) : !bookmaker ? (<div className="flex items-center gap-2 pt-0.5">
                        <Checkbox id="moedaConfirmacao" checked={moedaConfirmada} onCheckedChange={(checked) => setMoedaConfirmada(checked === true)}/>
              ...
              <FormSection icon={KeyRound} title="Credenciais de Acesso">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <Input value={loginUsername} onChange={(e) => setLoginUsername(e.target.value)} placeholder="username ou email" disabled={loading} className="h-9 text-sm"/>
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs">
                      Senha <span className="text-[10px] text-muted-foreground">{bookmaker ? "(deixe em branco para manter)" : "(opcional)"}</span>
                    </Label>
                    <PasswordInput value={loginPassword} onChange={setLoginPassword} placeholder={bookmaker ? "••••••••" : "senha"} disabled={loading}/>
                  </div>
                </div>
              </FormSection>
              ...
              <FormSection icon={StickyNote} title="Configuração">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5 sm:col-span-2">
                    ...
                    <Textarea value={observacoes} onChange={(e) => setObservacoes(e.target.value)} rows={2} placeholder="Notas internas..." disabled={loading} className="text-sm resize-none"/>
                  </div>
                </div>
              </FormSection>
            </div>
            ...
            <div className="sticky bottom-0 bg-background border-t px-5 py-3 flex gap-2.5">
              ...
              <Button type="submit" disabled={loading || isLoadingDetails || !parceiroId || !bookmakerId || !selectedLink || (!bookmaker && !moedaConfirmada)} className="flex-1 h-9">
    ...
    {parceiroId && (<ParceiroDialog open={showParceiroView} onClose={() => setShowParceiroView(false)} parceiro={{ id: parceiroId }} viewMode={true}/>)}
    </>);
}
