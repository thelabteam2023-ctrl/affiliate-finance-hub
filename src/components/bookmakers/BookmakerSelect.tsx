    ...
    >
      <div className="flex items-center justify-center gap-2 w-full">
        ...
        <div className="flex flex-col items-start truncate min-w-0 flex-1">
          <span className={cn(
            "uppercase text-sm font-medium leading-tight truncate w-full",
            isLimitada && "text-yellow-400"
          )}>
            {item.nome}
          </span>
          {item.instance_identifier && (
            <span className="text-[10px] text-muted-foreground font-normal mt-[1.5px] leading-tight uppercase">
              {item.instance_identifier}
            </span>
          )}
          {/* Modo saque: exibir nome curto do parceiro */}
          {isModoSaque && item.parceiro_nome && (
            <span className="text-[10px] text-muted-foreground/70 leading-tight mt-0.5 italic">
