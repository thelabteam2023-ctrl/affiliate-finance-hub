...
export interface BookmakerVinculoData {
  id: string;
  nome: string;
...
interface BookmakerVinculoProjetoSelectProps {
  projetoId: string;
  value: string;
  onValueChange: (value: string) => void;
  onBookmakerData?: (data: BookmakerVinculoData | null) => void;
  disabled?: boolean;
  placeholder?: string;
}

export interface BookmakerVinculoProjetoSelectRef {
  focus: () => void;
  open: () => void;
}

const BookmakerVinculoProjetoSelect = forwardRef<BookmakerVinculoProjetoSelectRef, BookmakerVinculoProjetoSelectProps>(({
  projetoId,
  value,
  onValueChange,
  onBookmakerData,
  ...
  const [items, setItems] = useState<BookmakerVinculoData[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [loading, setLoading] = useState(false);
  const [displayData, setDisplayData] = useState<BookmakerVinculoData | null>(null);
  ...
  // Buscar casas vinculadas ao projeto COM parceiro
  useEffect(() => {
    if (!projetoId) {
      setItems([]);
      return;
      ...
      try {
        const { data, error } = await supabase
          ...
          .select(`
            ...
            parceiro:parceiros!bookmakers_parceiro_id_fkey (
              id,
              nome
            )
          `)
        ...
        const mapped: BookmakerVinculoData[] = (data || []).map((b: any) => ({
          id: b.id,
          nome: b.nome,
        ...
        if (!abortController.signal.aborted) {
          console.error("Erro ao carregar vínculos do projeto:", error);
          setItems([]);
        }
      } finally {
        if (!abortController.signal.aborted) {
  ...
  // Filtrar itens pela busca (nome da casa ou parceiro)
  const filteredItems = items.filter((item) => {
    const search = searchTerm.toLowerCase();
    return (
      item.nome.toLowerCase().includes(search) ||
      (item.parceiro_nome?.toLowerCase().includes(search) ?? false)
    );
  });
...
BookmakerVinculoProjetoSelect.displayName = "BookmakerVinculoProjetoSelect";

export default BookmakerVinculoProjetoSelect;
