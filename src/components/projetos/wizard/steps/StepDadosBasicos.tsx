          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">Vincule um investidor para dividir lucros</p>
              </div>
            </div>

            <div className={cn(
              "grid gap-4",
              hasInvestidor ? "grid-cols-1 md:grid-cols-4" : "grid-cols-1"
            )}>
              <div className={cn(hasInvestidor ? "md:col-span-2" : "max-w-sm")}>
                <InvestidorSelect
                ...
                <>
                  <div className="space-y-1">
                    ...
                    <Input
                      ...
                      onChange={(e) => onChange({ percentual_investidor: parseFloat(e.target.value) || 0 })}
                      placeholder="Ex: 50"
                    />
                  </div>
                  <div className="space-y-1">
                    ...
                    <RadioGroup
                      ...
                      onValueChange={(value) => onChange({ base_calculo_investidor: value })}
                      className="flex flex-col gap-1"
                    >
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="LUCRO_LIQUIDO" id="lucro_liquido" />
                        <label htmlFor="lucro_liquido" className="text-xs cursor-pointer">Lucro Líquido</label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="LUCRO_BRUTO" id="lucro_bruto" />
                        <label htmlFor="lucro_bruto" className="text-xs cursor-pointer">Lucro Bruto</label>
                      </div>
                    </RadioGroup>
                  </div>
                </>
            ...
            {hasInvestidor && (
              <div className="p-2 rounded-lg bg-purple-500/10 border border-purple-500/20">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Receberá <strong className="text-purple-400">{formData.percentual_investidor || 0}%</strong> do{" "}
        ...
        <CardContent className="pt-4">
          <div className="flex items-start gap-3">
            <Checkbox
              ...
              onCheckedChange={(checked) => onChange({ tem_investimento_crypto: checked as boolean })}
            />
            <div className="space-y-1 flex-1">
              ...
              <p className="text-xs text-muted-foreground">Ativa conciliação patrimonial obrigatória</p>
            </div>
          </div>
        </CardContent>
      </Card>
