  ...
  if (isArbitragem) {
    try {
      const { data, error } = await supabase.rpc('criar_surebet_atomica', {
        ...
        p_estrategia: input.estrategia || 'SUREBET',
      ...
      return { success: true, data: { id: effectiveApostaId } };
    } catch (err: any) {
      return {
        success: false,
...
/**
 * Liquida uma perna individual de Surebet via RPC atômica.
 * 
 * MOTOR UNIFICADO v10: Toda lógica financeira está na RPC liquidar_perna_surebet_v1.
 * O service layer é apenas orquestrador — ZERO INSERT direto em financial_events.
 * 
 * A RPC cuida de:
 * 1. Validação e guard clause (resultado igual = no-op)
 * 2. Reversão de payout anterior (se re-liquidação)
 * 3. Criação de evento financeiro (PAYOUT/VOID_REFUND) com idempotência
 * 4. Atualização da perna em apostas_pernas
 * 5. Recálculo do status do registro pai
 */
export async function liquidarPernaSurebet(
  ...
  const { 
    surebet_id, perna_id, resultado, resultado_anterior,
    workspace_id, fonte_saldo = 'REAL'
  } = input;
  
  console.log("[ApostaService] Liquidando perna surebet via RPC:", { surebet_id, perna_id, resultado, resultado_anterior });
  ...
  try {
    ...
    if (error) {
      console.error("[ApostaService] Erro RPC liquidar_perna_surebet_v1:", error);
      return {
        success: false,
    ...
    const result = data as {
      ...
      lucro_prejuizo?: number;
      delta?: number;
      resultado_final_pai?: string;
    };

    if (!result?.success) {
      return {
        success: false,
        error: { code: 'LIQUIDATION_FAILED', message: result?.error || 'Falha ao liquidar perna' },
      };
    }

    console.log("[ApostaService] ✅ Perna surebet liquidada via RPC:", {
      perna_id, resultado,
      events_created: result.events_created,
      lucro_prejuizo: result.lucro_prejuizo,
      delta: result.delta,
    ...
    return {
      success: true,
      data: { 
        lucro_prejuizo: result.lucro_prejuizo ?? 0, 
...
export async function reliquidarAposta(
  ...
  try {
   ...
   if (!apostaAtual) {
    ...
    // ============================================================
    if (isArbitragem) {
      console.log("[ApostaService] Detectada Surebet/Arbitragem - usando orquestrador por perna");

      // 1. Buscar pernas (ordenadas) com workspace_id e moeda do bookmaker
      const { data: pernas, error: pernasError } = await supabase
        .from('apostas_pernas')
        .select('id, ordem, bookmaker_id, stake, odd, moeda, resultado, fonte_saldo, aposta_id')
        .eq('aposta_id', apostaId)
        .order('ordem', { ascending: true });

      if (pernasError || !pernas || pernas.length === 0) {
        console.error("[ApostaService] Surebet sem pernas em apostas_pernas:", pernasError);
        return {
          success: false,
          error: {
            code: 'SUREBET_NO_LEGS',
            message: 'Surebet não possui pernas registradas em apostas_pernas. ' +
                     'Re-liquidação só é possível para surebets com pernas estruturadas. ' +
                     'Use o menu "Liquidar" (Quick Resolve) por cenário em vez de "Liquidação Global".',
            details: { error: pernasError },
          },
        };
      }

      // 2. Buscar workspace_id da aposta pai
      const { data: apostaPai } = await supabase
        .from('apostas_unificada')
        .select('workspace_id')
        .eq('id', apostaId)
        .single();

      if (!apostaPai?.workspace_id) {
        return {
          success: false,
          error: {
            code: 'APOSTA_NO_WORKSPACE',
            message: 'Aposta sem workspace_id (registro corrompido).',
          },
        };
      }
