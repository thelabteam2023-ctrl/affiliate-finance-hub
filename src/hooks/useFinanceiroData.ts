...
async function fetchFinanceiroData(workspaceId: string): Promise<FinanceiroData> {
  ...
  const [
    ...
    supabase.from("cash_ledger").select("tipo_transacao, valor, valor_confirmado, tipo_moeda, data_transacao, moeda, origem_tipo, projeto_id_snapshot, status").eq("workspace_id", workspaceId).eq("status", "CONFIRMADO").is("reversed_at", null).limit(10000),
