...
async function fetchFinanceiroData(workspaceId: string): Promise<FinanceiroData> {
  ...
  const [
    ...
    supabase.from("despesas_administrativas").select("*").eq("workspace_id", workspaceId).eq("status", "CONFIRMADO").limit(10000),
    supabase.from("despesas_administrativas").select("*").eq("workspace_id", workspaceId).eq("status", "PENDENTE").limit(10000),
