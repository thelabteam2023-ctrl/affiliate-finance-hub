...
export function calculateDeterministicHedge(config: ExtractionConfig): StrategyResults {
  ...
  // When back loses at event i: we win lay i (minus commission), but paid liabilities 0..i-1
  ...
  // When all events win (failure): freebet pays out, but all lays lost (no commission)
