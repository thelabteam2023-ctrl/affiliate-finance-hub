DROP FUNCTION IF EXISTS public.get_projetos_lucro_operacional(uuid[], text, text, jsonb);

CREATE FUNCTION public.get_projetos_lucro_operacional(
  p_projeto_ids uuid[],
  ...
  FOREACH v_proj IN ARRAY p_projeto_ids LOOP
    ...
    SELECT jsonb_build_object(
      ...
      'bonus', (
        ...
        FROM (
          SELECT
            '__CONSOLIDADO__' as moeda,
            SUM(
              COALESCE(
                bl.valor_consolidado_snapshot,
                (
                  COALESCE(bl.bonus_amount, 0) * CASE UPPER(COALESCE(bl.currency, b.moeda, 'BRL'))
                    WHEN 'BRL' THEN 1
                    WHEN 'USD' THEN v_cot_usd
                    WHEN 'USDT' THEN v_cot_usd
                    WHEN 'USDC' THEN v_cot_usd
                    WHEN 'EUR' THEN v_cot_eur
                    WHEN 'GBP' THEN v_cot_gbp
                    WHEN 'MYR' THEN v_cot_myr
                    WHEN 'MXN' THEN v_cot_mxn
                    ...
                    ELSE 1
                  END
                ) / NULLIF(v_taxa_consolidacao_brl, 0)
              )
            )::numeric as total
    ...
    FOR v_modulo IN SELECT jsonb_object_keys(v_proj_result) LOOP
      ...
      FOR v_moeda IN SELECT jsonb_object_keys(v_modulo_data) LOOP
        ...
        -- Valor JA consolidado na moeda do projeto (snapshot congelado). Taxa = 1.
        IF v_moeda IN ('__CONSOLIDADO__', '__SNAPSHOT_USD__') THEN
          ...
          v_acc := COALESCE((v_por_moeda->>UPPER(v_moeda_consolidacao))::numeric, 0);
          IF v_modulo IN ('perdas', 'cancelamento_bonus') THEN
            v_acc := v_acc - v_valor;
          ELSE
            v_acc := v_acc + v_valor;
          END IF;
          v_por_moeda := jsonb_set(v_por_moeda, ARRAY[UPPER(v_moeda_consolidacao)], to_jsonb(v_acc));
          CONTINUE;
        END IF;
...
GRANT EXECUTE ON FUNCTION public.get_projetos_lucro_operacional(uuid[], text, text, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_projetos_lucro_operacional(uuid[], text, text, jsonb) TO service_role;