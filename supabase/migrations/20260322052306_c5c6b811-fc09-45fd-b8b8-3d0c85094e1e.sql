...
BEGIN
  ...
  WITH
  apostas_daily AS (
    SELECT (a.data_aposta AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
    ...
    WHERE a.projeto_id = p_projeto_id AND a.status = 'LIQUIDADA'
  ...
  giros_daily AS (
    SELECT (g.data_registro AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
    ...
    FROM giros_gratis g JOIN bookmakers b ON b.id = g.bookmaker_id
  ...
  bonus_daily AS (
    SELECT (bl.credited_at AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
  ...
  conciliacoes_daily AS (
    SELECT (ba.created_at AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
      CASE WHEN UPPER(COALESCE(b.moeda,'BRL')) IN ('USDT','USDC') THEN 'USD'
           ELSE UPPER(COALESCE(b.moeda,'BRL')) END AS moeda,
    ...
    WHERE ba.origem = 'CONCILIACAO_VINCULO' AND ba.referencia_tipo = 'projeto'
  ...
  ajustes_daily AS (
    SELECT (cl.data_transacao AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
    ...
    FROM cash_ledger cl
    WHERE cl.status = 'CONFIRMADO' AND cl.tipo_transacao = 'AJUSTE_SALDO'
      AND cl.projeto_id_snapshot = p_projeto_id AND COALESCE(cl.valor,0) != 0
  ...
  fx_daily AS (
    SELECT (cl.data_transacao AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
      CASE WHEN UPPER(COALESCE(cl.moeda,'BRL')) IN ('USDT','USDC') THEN 'USD'
           ELSE UPPER(COALESCE(cl.moeda,'BRL')) END AS moeda,
      CASE WHEN cl.tipo_transacao = 'PERDA_CAMBIAL' THEN -COALESCE(cl.valor,0) ELSE COALESCE(cl.valor,0) END AS val
    FROM cash_ledger cl
    WHERE cl.status = 'CONFIRMADO' AND cl.tipo_transacao IN ('GANHO_CAMBIAL','PERDA_CAMBIAL')
      AND cl.projeto_id_snapshot = p_projeto_id AND COALESCE(cl.valor,0) != 0
      AND (v_start_ts IS NULL OR cl.data_transacao >= v_start_ts)
      AND (v_end_ts IS NULL OR cl.data_transacao <= v_end_ts)
  ),
  ...
  perdas_daily AS (
    SELECT (pp.data_registro AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
      CASE WHEN UPPER(COALESCE(b.moeda,'BRL')) IN ('USDT','USDC') THEN 'USD'
           ELSE UPPER(COALESCE(b.moeda,'BRL')) END AS moeda,
      -COALESCE(pp.valor,0) AS val
    FROM projeto_perdas pp LEFT JOIN bookmakers b ON b.id = pp.bookmaker_id
  ...
  perdas_cancel_daily AS (
    ...
    FROM cash_ledger cl
    WHERE cl.ajuste_motivo = 'BONUS_CANCELAMENTO' AND cl.ajuste_direcao = 'SAIDA'
