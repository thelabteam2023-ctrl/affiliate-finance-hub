...
BEGIN
  ...
  -- IDs de apostas que JÁ tiveram STAKE debitado via financial_events
  ...
  -- Stakes de apostas simples PENDENTES que NÃO foram debitadas via eventos (SOMENTE SALDO REAL)
  ...
  -- Stakes de pernas PENDENTES que NÃO foram debitadas via eventos (SOMENTE SALDO REAL)
  apostas_pernas_pendentes AS (
    SELECT
      ap.bookmaker_id,
      COALESCE(SUM(
        CASE
          WHEN ap.fonte_saldo = 'FREEBET' THEN 0
          ELSE COALESCE(ap.stake, 0)
        END
      ), 0) AS total_stake
  ...
  -- Consolidar stakes pendentes de saldo REAL (para cálculo de saldo_disponivel)
  ...
  -- Calcular saldo_em_aposta TOTAL INFORMATIVO (inclui TODAS as stakes pendentes, inclusive freebet)
  todas_apostas_pendentes AS (
    SELECT bookmaker_id, SUM(total_stake) AS total_stake
    FROM (
      SELECT au.bookmaker_id, COALESCE(SUM(COALESCE(au.stake, 0)), 0) AS total_stake
      FROM public.apostas_unificada au
      WHERE au.workspace_id = v_workspace_id AND au.status = 'PENDENTE'
        AND au.cancelled_at IS NULL AND au.bookmaker_id IS NOT NULL
        AND (p_projeto_id IS NULL OR au.projeto_id = p_projeto_id)
      GROUP BY au.bookmaker_id
      UNION ALL
      SELECT ap.bookmaker_id, COALESCE(SUM(COALESCE(ap.stake, 0)), 0) AS total_stake
      FROM public.apostas_pernas ap
      INNER JOIN public.apostas_unificada au ON au.id = ap.aposta_id
      WHERE au.workspace_id = v_workspace_id AND au.status = 'PENDENTE'
        AND au.cancelled_at IS NULL AND au.bookmaker_id IS NULL
        AND (p_projeto_id IS NULL OR au.projeto_id = p_projeto_id)
      GROUP BY ap.bookmaker_id
    ) all_combined
    GROUP BY bookmaker_id
  ...
  SELECT
    ...
    -- saldo_em_aposta: total informativo INCLUINDO freebet (para exibição completa)
    COALESCE(tap.total_stake, 0)::NUMERIC AS saldo_em_aposta,
    -- saldo_disponivel: só deduz stakes REAIS de apostas SEM evento STAKE (não debitadas)
