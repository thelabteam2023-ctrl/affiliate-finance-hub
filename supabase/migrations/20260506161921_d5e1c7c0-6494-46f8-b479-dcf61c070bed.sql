...
BEGIN
  ...
  IF p_perna_id_ajuste IS NOT NULL THEN
    SELECT bookmaker_id, moeda, lucro_prejuizo 
    INTO v_bookmaker_id, v_moeda, v_perna_old_lp
    FROM public.apostas_pernas
    WHERE id = p_perna_id_ajuste AND aposta_id = p_aposta_id;

    -- Atualizar o lucro da perna para manter consistência
    IF FOUND THEN
      UPDATE public.apostas_pernas 
      SET lucro_prejuizo = COALESCE(lucro_prejuizo, 0) + v_delta,
          updated_at = NOW()
      WHERE id = p_perna_id_ajuste;
    END IF;
  ELSE
