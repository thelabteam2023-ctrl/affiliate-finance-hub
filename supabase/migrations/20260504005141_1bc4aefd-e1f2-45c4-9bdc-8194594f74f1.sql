...
BEGIN
  -- Definir contexto para evitar loops de recalculo se houver triggers
  PERFORM set_config('app.surebet_recalc_context', 'on', true);

  -- 1. Obter dados da perna
  ...
  -- Lock na aposta pai para consistência
  PERFORM 1 FROM apostas_unificada WHERE id = v_surebet_id FOR UPDATE;

  -- Se o resultado for o mesmo, não faz nada
  ...
  -- 2. VALIDAÇÃO CRÍTICA DO LEDGER (Apenas se estiver saindo de PENDENTE para um resultado real)
  -- Importante: Incluir AJUSTE pois edições de stake geram eventos de AJUSTE e não STAKE puro
  IF p_resultado IS NOT NULL AND p_resultado NOT IN ('PENDENTE', '') THEN
    ...
    WHERE fe.aposta_id = v_surebet_id
      ...
      AND fe.tipo_evento IN ('STAKE', 'FREEBET_STAKE', 'AJUSTE')
  ...
  -- 3. Reverter eventos de liquidação anteriores se existirem (para re-liquidação)
  IF v_old_resultado IS NOT NULL AND v_old_resultado NOT IN ('PENDENTE', '') THEN
    ...
    BEGIN
      -- Procurar pelo payout/refund anterior desta perna
  ...
  -- 4. Calcular novo payout e criar evento
  ...
  -- 6. Verificar se todas as pernas estão liquidadas para atualizar status da aposta pai
  ...
  IF v_todas_liquidadas THEN
    -- Calcular resultado final simplificado para a aposta pai
    ...
    -- Usar função de recalculo para garantir PL consolidado e moedas
