...
BEGIN
  ...
  bonus_creditados AS (
    SELECT 
      pblb.bookmaker_id,
      -- CORREÇÃO: Excluir FREEBET do saldo_bonus pois já está em bookmakers.saldo_freebet
      COALESCE(SUM(CASE WHEN pblb.tipo_bonus != 'FREEBET' THEN pblb.saldo_atual ELSE 0 END), 0) AS total_bonus,
