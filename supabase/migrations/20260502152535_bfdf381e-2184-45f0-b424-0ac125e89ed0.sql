-- 1. Corrigir a função de reversão para incluir campos de cripto e cotação
...
BEGIN
  ...
  INSERT INTO public.cash_ledger (
    ...
    v_tx.valor_destino, v_tx.valor_origem, -- Swapped because it's a mirror
    v_tx.cotacao, v_tx.cotacao_implicita,
    v_tx.cotacao_destino_usd, v_tx.cotacao_origem_usd, -- Swapped
    'CONFIRMED'
...
-- 2. Corrigir os dados da transação inconsistente
-- Transação original: 2416ed1d-0f94-453c-b81e-8f6f639820a0
-- Transação de reversão incompleta: 6a83cd88-d0bb-4e63-b41a-95ef597df578
UPDATE public.cash_ledger
SET coin = 'USDT',
    qtd_coin = 503.8,
    valor_usd = 503.8,
    ...
    cotacao = 1,
    cotacao_implicita = 1,
    cotacao_origem_usd = 1,
    cotacao_destino_usd = 1,
    transit_status = 'CONFIRMED',
    updated_at = now()
WHERE id = '6a83cd88-d0bb-4e63-b41a-95ef597df578'
  AND tipo_transacao = 'AJUSTE_RECONCILIACAO'
