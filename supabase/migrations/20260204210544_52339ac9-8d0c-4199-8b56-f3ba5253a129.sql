-- =================================================================
-- CORREÇÃO DEFINITIVA: reliquidar_aposta_v5
-- Corrige colunas: event_type → tipo_evento, user_id → created_by
-- Adiciona campo obrigatório: origem
-- =================================================================
...
BEGIN
  ...
  IF v_valor_reversao > 0 AND v_bookmaker_id IS NOT NULL THEN
    ...
    INSERT INTO financial_events (
      bookmaker_id,
      aposta_id,
      tipo_evento,     -- CORRIGIDO: era event_type
      tipo_uso,
      origem,          -- ADICIONADO: campo obrigatório
      valor,
      moeda,
      workspace_id,
      created_by,      -- CORRIGIDO: era user_id
      idempotency_key,
      metadata,
      processed_at
    ) VALUES (
  ...
  IF v_valor_novo_payout > 0 AND v_bookmaker_id IS NOT NULL THEN
    ...
    INSERT INTO financial_events (
      ...
      v_bookmaker_id,
      p_aposta_id,
      CASE WHEN p_novo_resultado IN ('VOID', 'MEIO_RED') THEN 'VOID_REFUND' ELSE 'PAYOUT' END,
      v_tipo_uso,
      CASE WHEN p_novo_resultado IN ('VOID', 'MEIO_RED') THEN 'AJUSTE' ELSE 'LUCRO' END,
...
'v9.5.1 - Reliquida aposta usando financial_events (colunas corrigidas: tipo_evento, created_by, origem)';