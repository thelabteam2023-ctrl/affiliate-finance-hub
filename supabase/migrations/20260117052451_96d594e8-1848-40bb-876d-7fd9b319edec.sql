...
-- CORREÇÃO FINAL: Incluir EXTRACAO_BONUS nas estratégias com pernas JSONB
-- Também incluir outras estratégias que podem usar pernas
...
BEGIN
  ...
  -- =====================================================
  -- Bets P/L: Apostas diretas (bookmaker_id direto)
  -- =====================================================
  ...
  -- =====================================================
  -- CORREÇÃO: Pernas JSONB para TODAS as estratégias que usam pernas
  -- Inclui: SUREBET, ARBITRAGEM, EXTRACAO_BONUS e qualquer outra
  -- =====================================================
  ...
  WHERE a.pernas IS NOT NULL
    AND jsonb_array_length(a.pernas) > 0
    AND a.status = 'LIQUIDADA'
    AND (perna->>'bookmaker_id')::uuid = p_bookmaker_id
    AND (perna->>'lucro_prejuizo') IS NOT NULL;
