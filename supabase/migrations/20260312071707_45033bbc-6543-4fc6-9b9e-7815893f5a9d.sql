...
BEGIN
  ...
  -- Get bookmaker IDs for this project
    ...
    'depositos', (
      ...
      FROM (
        ...
        WHERE tipo_transacao IN ('DEPOSITO', 'DEPOSITO_VIRTUAL')
          ...
          AND (
            projeto_id_snapshot = p_projeto_id::text
    ...
    'saques', (
      ...
      FROM (
        ...
        WHERE tipo_transacao IN ('SAQUE', 'SAQUE_VIRTUAL')
          AND status = 'CONFIRMADO'
          AND projeto_id_snapshot = p_projeto_id::text
      ) s
    ),
    ...
    'ledger_extras', (
      ...
      FROM (
        ...
        WHERE status = 'CONFIRMADO'
          AND (
            (tipo_transacao = 'AJUSTE_SALDO' AND projeto_id_snapshot = p_projeto_id::text)
            OR (tipo_transacao IN ('GANHO_CAMBIAL', 'PERDA_CAMBIAL') AND projeto_id_snapshot = p_projeto_id::text)
