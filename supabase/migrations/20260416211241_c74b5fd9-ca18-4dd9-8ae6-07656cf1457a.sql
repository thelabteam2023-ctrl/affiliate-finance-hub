...
WITH freebets_base AS (
  SELECT 
    ...
    b.projeto_id,
    GREATEST(b.saldo_freebet, 0)::numeric AS saldo_fisico,
    EXISTS (
...
freebets_ativas_lifo AS (
  -- LIFO: mais nova primeiro absorve o saldo físico
  SELECT 
    fb.*,
    SUM(fb.valor) OVER (
      PARTITION BY fb.bookmaker_id 
      ORDER BY fb.data_recebida DESC, fb.created_at DESC
...
'LIFO: distribui saldo_freebet físico priorizando freebets MAIS RECENTES (refletindo que antigas já foram consumidas em apostas históricas). Canceladas via FREEBET_EXPIRE têm status CANCELADA.';
