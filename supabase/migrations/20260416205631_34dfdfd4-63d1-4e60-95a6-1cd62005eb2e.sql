...
WITH consumo_por_bookmaker AS (
  SELECT 
    ...
    SUM(
      CASE 
        WHEN fe.tipo_evento = 'STAKE' THEN fe.valor
        WHEN fe.tipo_evento = 'REVERSAL' AND fe.reversed_event_id IS NOT NULL THEN -fe.valor
        ELSE 0
      END
    ) AS total_consumido
...
'View derivada do ledger com FIFO: cada freebet calcula valor_restante descontando STAKEs do financial_events das freebets mais antigas primeiro (data_recebida ASC).';
