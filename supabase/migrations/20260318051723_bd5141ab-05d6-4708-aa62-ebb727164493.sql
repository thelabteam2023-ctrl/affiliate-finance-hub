...
DROP FUNCTION IF EXISTS public.get_central_operacoes_data(uuid, uuid, boolean, boolean, boolean);

CREATE FUNCTION public.get_central_operacoes_data(
  p_workspace_id uuid,
  p_user_id uuid,
...
BEGIN
  ...
  IF p_include_partner THEN
    ...
    -- 6. Comissões pendentes (check both comissao_paga flag AND movimentacoes)
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'parceriaId', p.id,
      'parceiroNome', par.nome,
    ...
    -- 8. Parceiros sem parceria (EXCLUDE caixa operacional)
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', par.id,
      'nome', par.nome,
      'cpf', par.cpf,
