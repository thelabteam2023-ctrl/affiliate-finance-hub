...
BEGIN
  ...
  v_valor_perda := ABS(COALESCE(v_ledger.valor_usd, 0));
  ...
  -- 4) Registrar em projeto_perdas usando as colunas reais da tabela
  IF v_projeto IS NOT NULL THEN
    INSERT INTO projeto_perdas (
      workspace_id, user_id, projeto_id, bookmaker_id,
      valor, categoria, descricao, status, data_registro
    ) VALUES (
      v_workspace, v_user, v_projeto, NULL,
      v_valor_perda,
      'PERDA_TRANSIT_'||v_motivo_norm,
      format('Perda em transito: %s | rede=%s | hash=%s | obs=%s | ajuste_ledger=%s | ledger_origem=%s',
        v_motivo_norm,
        COALESCE(p_rede,'-'),
        COALESCE(p_hash,'-'),
  ...
  RETURN json_build_object(
    ...
    'valor_perdido_usd', v_valor_perda,
    'motivo', v_motivo_norm
  );
END;
$function$;