-- Corrige competência do bônus restaurado do Diego/EVERYGAME no projeto destino
UPDATE public.project_bookmaker_link_bonuses
SET credited_at = COALESCE(
      (
        SELECT (cl.data_transacao::date::timestamp AT TIME ZONE 'America/Sao_Paulo')
        FROM public.cash_ledger cl
        WHERE cl.destino_bookmaker_id = '8de2ba2c-011b-49f4-970e-be8637a9b05e'
          AND cl.projeto_id_snapshot = '438cef89-4a9a-4e72-8bc9-b1c3d7dc9693'
          AND cl.tipo_transacao = 'DEPOSITO_VIRTUAL'
    ...
    updated_at = now(),
    notes = COALESCE(notes, '') || E'\n[Remediação 2026-04-23: competência do bônus ajustada para o projeto destino.]'
WHERE id = 'fe8c6992-a0ff-490e-b10d-bb914dcebbab';

-- Regra sistêmica: bônus migrado entre projetos deve contar na competência do projeto destino
  ...
  IF v_dv_valor > 0 THEN
    INSERT INTO cash_ledger (
      ...
      jsonb_build_object(
        ...
        'trigger_version', 'real_only_migracao_v3_destination_competence'
      )
    );
  END IF;
