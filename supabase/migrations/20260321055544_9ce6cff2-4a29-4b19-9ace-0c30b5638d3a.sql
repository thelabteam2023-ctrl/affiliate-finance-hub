...
BEGIN
  ...
  SELECT COALESCE(SUM(
    CASE
      WHEN valor_brl_referencia IS NOT NULL AND v_moeda_consolidacao = 'BRL' THEN valor_brl_referencia
      WHEN COALESCE(currency, 'BRL') = v_moeda_consolidacao THEN COALESCE(bonus_amount, 0)
      WHEN COALESCE(currency, 'BRL') IN ('USD','USDT','USDC') AND v_moeda_consolidacao = 'BRL' THEN COALESCE(bonus_amount, 0) * v_cotacao
      WHEN COALESCE(currency, 'BRL') = 'BRL' AND v_moeda_consolidacao IN ('USD','USDT','USDC') THEN COALESCE(bonus_amount, 0) / v_cotacao
      ELSE COALESCE(bonus_amount, 0)
    END
  ), 0)
  INTO v_lucro_bonus
