...
BEGIN
  ...
  IF v_has_entries THEN
    FOR v_entry IN
      SELECT ae.moeda, ae.stake, ae.odd, ap.resultado, ae.fonte_saldo,
             LOWER(COALESCE(ap.tipo,'back')) AS tipo,
             ap.cotacao_snapshot AS snap
      FROM public.apostas_perna_entradas ae
      JOIN public.apostas_pernas ap ON ap.id = ae.perna_id
      WHERE ap.aposta_id = NEW.id
    LOOP
      v_entry_moeda := UPPER(COALESCE(v_entry.moeda, 'BRL'));
      IF v_entry_moeda != v_moeda_consolidacao THEN
        v_is_multi := true;
      END IF;
      ...
      -- Hierarquia: snapshot congelado por perna (USD↔BRL) > Cotação de Trabalho > default
      IF v_entry_moeda = v_moeda_consolidacao THEN
        v_rate := 1;
      ELSIF v_snap IS NOT NULL AND v_snap > 0 THEN
        IF v_entry_moeda IN ('USD','USDT','USDC') AND v_moeda_consolidacao = 'BRL' THEN
          v_rate := v_snap;
        ELSIF v_entry_moeda = 'BRL' AND v_moeda_consolidacao = 'USD' THEN
          v_rate := 1 / v_snap;
        END IF;
      END IF;

      IF v_rate IS NULL THEN
        v_brl_rate_from := COALESCE((v_rates->>v_entry_moeda)::NUMERIC, 1);
        v_brl_rate_to   := COALESCE((v_rates->>UPPER(v_moeda_consolidacao))::NUMERIC, 1);
        v_rate := CASE WHEN v_brl_rate_to > 0 THEN v_brl_rate_from / v_brl_rate_to ELSE 1 END;
      END IF;
  ...
  ELSIF v_has_pernas THEN
    FOR v_entry IN
      SELECT moeda, COALESCE(lucro_prejuizo, 0) AS lp, resultado, cotacao_snapshot AS snap
  ...
  ELSE
    ...
    v_snap := NEW.cotacao_snapshot;
    v_rate := NULL;
    IF v_simples_moeda = v_moeda_consolidacao THEN
      v_rate := 1;
    ELSIF v_snap IS NOT NULL AND v_snap > 0 THEN
      IF v_simples_moeda IN ('USD','USDT','USDC') AND v_moeda_consolidacao = 'BRL' THEN
        v_rate := v_snap;
      ELSIF v_simples_moeda = 'BRL' AND v_moeda_consolidacao = 'USD' THEN
        v_rate := 1 / v_snap;
      END IF;
    END IF;
