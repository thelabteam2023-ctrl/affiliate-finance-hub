...
BEGIN
  ...
  FOREACH v_proj IN ARRAY p_projeto_ids LOOP
    ...
    -- Buscar cotações de trabalho do projeto
    SELECT 
    ...
    -- Override por projeto se fornecido
    ...
    -- Taxa da moeda de consolidação em BRL (para cross-rate quando consolidação != BRL)
    ...
    SELECT jsonb_build_object(
      ...
      -- BÔNUS: usar valor_consolidado_snapshot (USD congelado no crédito) — paridade com Visão Geral
      -- Marcamos a moeda como '__SNAPSHOT_USD__' para sinalizar que já está em USD
      -- e não deve passar por conversão normal (será tratado abaixo).
      'bonus', (
        SELECT COALESCE(jsonb_object_agg(moeda, total), '{}'::jsonb)
        FROM (
          SELECT
            ...
            SUM(
              COALESCE(
                ...
                -- fallback: se não houver snapshot, usar bonus_amount na moeda original convertida
                CASE 
      ...
      'perdas', (
        ...
        FROM (
          ...
          WHERE pp.projeto_id = v_proj AND pp.status = 'CONFIRMADA'
            ...
            AND (v_start_ts IS NULL OR pp.data_perda >= v_start_ts)
            AND (v_end_ts IS NULL OR pp.data_perda <= v_end_ts)
          GROUP BY 1
        ) sub
      ),
      'conciliacao', (
    ...
    -- Calcular consolidado e por moeda usando cotações de trabalho
    v_consolidado := 0;
    v_por_moeda := '{}'::jsonb;
    ...
    FOR v_modulo IN SELECT jsonb_object_keys(v_proj_result) LOOP
      ...
      FOR v_moeda IN SELECT jsonb_object_keys(v_modulo_data) LOOP
        ...
        -- Tratamento especial: bônus já vem em USD via snapshot
        IF v_moeda = '__SNAPSHOT_USD__' THEN
          -- v_valor está em USD; converter para moeda de consolidação
          IF UPPER(v_moeda_consolidacao) = 'USD' THEN
            v_taxa := 1;
          ELSIF UPPER(v_moeda_consolidacao) = 'BRL' THEN
            v_taxa := v_cot_usd;
          ELSE
            -- USD -> BRL -> consolidação
        ...
        -- Determinar taxa: moeda original -> consolidação
        IF UPPER(v_moeda) = UPPER(v_moeda_consolidacao) THEN
          v_taxa := 1;
        ELSE
          -- Pega taxa da moeda em BRL
        ...
        -- Aplicar sinal: perdas e cancelamento são subtrações
        IF v_modulo IN ('perdas', 'cancelamento_bonus') THEN
          v_consolidado := v_consolidado - (v_valor * COALESCE(v_taxa, 1));
        ELSE
          v_consolidado := v_consolidado + (v_valor * COALESCE(v_taxa, 1));
        ...
        -- Acumular por moeda original (com sinal)
