...
BEGIN
  ...
  SELECT
    ...
    -- saldo_disponivel: saldo real - stakes NÃO debitadas - saques pendentes (clamp >= 0)
    GREATEST(ba.saldo_base - COALESCE(ct.total_stake_real, 0) - COALESCE(sp.total_saque_pendente, 0), 0) AS saldo_disponivel,
    -- saldo_operavel: saldo disponível + GREATEST(0, freebet) + bonus
    -- CORREÇÃO: freebet negativo NÃO deve contaminar o saldo operável
    GREATEST(ba.saldo_base - COALESCE(ct.total_stake_real, 0) - COALESCE(sp.total_saque_pendente, 0), 0)
...
-- Também corrigir a view v_bookmaker_saldo_operavel
...
SELECT id,
    ...
    GREATEST(COALESCE(saldo_freebet, 0::numeric), 0) AS saldo_freebet,
    saldo_atual + GREATEST(COALESCE(saldo_freebet, 0::numeric), 0) AS saldo_operavel,
    status
   FROM bookmakers b
