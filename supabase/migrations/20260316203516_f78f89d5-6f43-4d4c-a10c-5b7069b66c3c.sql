...
BEGIN
  ...
  -- Stakes de apostas simples PENDENTES que NÃO foram debitadas via eventos
  ...
  -- Stakes de pernas PENDENTES que NÃO foram debitadas via eventos
  ...
  -- Consolidar ambas as fontes de stakes pendentes (somente não-debitadas)
  ...
  -- Calcular saldo_em_aposta TOTAL (para exibição informativa)
  todas_apostas_pendentes AS (
    SELECT bookmaker_id, SUM(total_stake) AS total_stake
    FROM (
      SELECT au.bookmaker_id, COALESCE(SUM(
        CASE WHEN au.fonte_saldo = 'FREEBET' OR au.usar_freebet = TRUE THEN 0
             ELSE COALESCE(au.stake, 0) END
      ), 0) AS total_stake
      FROM public.apostas_unificada au
      ...
      GROUP BY au.bookmaker_id
      UNION ALL
      SELECT ap.bookmaker_id, COALESCE(SUM(
        CASE WHEN ap.fonte_saldo = 'FREEBET' THEN 0
             ELSE COALESCE(ap.stake, 0) END
      ), 0) AS total_stake
      FROM public.apostas_pernas ap
      INNER JOIN public.apostas_unificada au ON au.id = ap.aposta_id
      WHERE au.workspace_id = v_workspace_id AND au.status = 'PENDENTE'
  ...
  SELECT
    ...
    -- saldo_em_aposta: total informativo (para exibição)
    COALESCE(tap.total_stake, 0)::NUMERIC AS saldo_em_aposta,
    -- saldo_disponivel: só deduz stakes de apostas SEM evento STAKE (não debitadas)
