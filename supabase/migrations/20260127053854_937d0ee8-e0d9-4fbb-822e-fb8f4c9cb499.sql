...
-- Corrigir RPC reset_projeto_operacional_seguro usando recalcular_saldos_projeto
...
BEGIN
  ...
  -- ============================================================
  -- RECÁLCULO DE SALDOS via v_financial_audit
  -- Atualiza saldo para refletir a soma dos eventos financeiros
  -- ============================================================
  IF NOT p_dry_run THEN
    FOR v_bookmaker IN
      SELECT b.id, b.nome, 
             COALESCE(fa.soma_eventos_normal, 0) as saldo_calculado,
             COALESCE(fa.soma_eventos_freebet, 0) as freebet_calculado
      FROM bookmakers b
      LEFT JOIN v_financial_audit fa ON fa.bookmaker_id = b.id
      WHERE b.projeto_id = p_projeto_id
    LOOP
      -- Atualizar saldo para refletir soma dos eventos
      UPDATE bookmakers
      SET saldo_atual = v_bookmaker.saldo_calculado,
          saldo_freebet = v_bookmaker.freebet_calculado,
          updated_at = NOW()
