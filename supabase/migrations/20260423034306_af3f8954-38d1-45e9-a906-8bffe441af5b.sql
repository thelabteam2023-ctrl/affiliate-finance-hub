-- ============================================================
-- MIGRAÇÃO DE FREEBETS DE ESTOQUE ENTRE PROJETOS
-- Estados suportados:
--   A) Não usada       → migração total (move o registro inteiro)
--   B) Totalmente usada → bloqueada (preserva histórico)
--   C) Parcialmente usada → migração com quebra (cria filha com saldo restante)
-- ============================================================

-- 1. Campos de auditoria/rastreabilidade na tabela freebets_recebidas
ALTER TABLE public.freebets_recebidas
  ADD COLUMN IF NOT EXISTS migrada_de_freebet_id uuid REFERENCES public.freebets_recebidas(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS migrada_para_freebet_id uuid REFERENCES public.freebets_recebidas(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS migrada_de_projeto_id uuid REFERENCES public.projetos(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS migrada_para_projeto_id uuid REFERENCES public.projetos(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS migrated_at timestamp with time zone,
  ADD COLUMN IF NOT EXISTS migrated_by uuid,
  ADD COLUMN IF NOT EXISTS valor_original_referencia numeric,
  ADD COLUMN IF NOT EXISTS valor_consumido_no_origem numeric;

CREATE INDEX IF NOT EXISTS idx_freebets_recebidas_migrada_de
  ON public.freebets_recebidas(migrada_de_freebet_id)
  WHERE migrada_de_freebet_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_freebets_recebidas_migrada_para
  ON public.freebets_recebidas(migrada_para_freebet_id)
  WHERE migrada_para_freebet_id IS NOT NULL;

-- 2. RPC principal de migração
...
BEGIN
  ...
  -- 2.1 Carregar freebet origem
  SELECT * INTO v_freebet
  FROM public.freebets_recebidas
  ...
  SELECT id, nome, workspace_id INTO v_origem_projeto
  FROM public.projetos WHERE id = v_freebet.projeto_id;

  SELECT id, nome, workspace_id INTO v_destino_projeto
  FROM public.projetos WHERE id = p_destino_projeto_id;
  ...
  -- 2.4 Validar bookmaker (deve estar vinculado ao projeto destino)
  SELECT id, nome, projeto_id, workspace_id INTO v_bookmaker
  FROM public.bookmakers WHERE id = v_freebet.bookmaker_id;
  ...
  -- 2.5 Calcular consumo no projeto de origem (apostas que usaram esta freebet)
  --    Soma stake_freebet das pernas vinculadas a apostas do projeto de origem.
  ...
  -- valor_restante já é mantido pelo sistema (derivado do ledger). Usamos como fonte autoritativa.
  v_valor_restante := COALESCE(
    (SELECT valor_restante FROM (
      SELECT public.calcular_valor_restante_freebet(p_freebet_id) AS valor_restante
    ) sub),
    v_freebet.valor
  );

  -- Se a função helper não existir, fallback: assume valor total como restante
  ...
  -- 2.6 Determinar estado
  IF v_apostas_no_origem = 0 AND v_valor_consumido = 0 THEN
    v_estado := 'TOTAL'; -- Estado A
  ELSIF v_valor_restante <= 0 THEN
    v_estado := 'ESGOTADA'; -- Estado B
  ELSE
    v_estado := 'PARCIAL'; -- Estado C
  END IF;

  -- 2.7 Bloqueio de Estado B
  ...
  -- 2.8 Migração — Estado A (total): mover registro
  ...
  ELSE
    -- 2.9 Migração — Estado C (parcial): congelar original e criar filha
  ...
  INSERT INTO public.audit_logs (
    ...
    jsonb_build_object(
      'tipo', 'freebet_migration',
      'estado', v_estado,
      'valor_original', v_freebet.valor,
      'valor_restante', v_valor_restante,
      'valor_consumido', v_valor_consumido,
      'projeto_origem', v_origem_projeto.nome,
      'projeto_destino', v_destino_projeto.nome
    )
  );
  ...
  RETURN jsonb_build_object(
    'success', true,
    'estado', v_estado,
    'freebet_id_origem', p_freebet_id,
    'freebet_id_destino', v_nova_freebet_id,
...
-- 3. RPC auxiliar: pré-validação para a UI (saber estado antes de clicar)
...
BEGIN
  ...
  v_valor_restante := COALESCE(public.calcular_valor_restante_freebet(p_freebet_id), v_freebet.valor);
  v_valor_consumido := GREATEST(v_freebet.valor - v_valor_restante, 0);
