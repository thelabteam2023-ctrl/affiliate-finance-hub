-- ============================================================
-- FIX 1: reliquidar_aposta_v6 — checagem dupla de chave de payout
-- ============================================================
      ...
      IF v_perna.bookmaker_id IS NOT NULL THEN
        ...
        IF v_resultado_anterior IS NOT NULL AND v_resultado_anterior <> 'PENDENTE' THEN
          ...
          IF v_perna_payout > 0 AND v_perna_tipo_evento IS NOT NULL THEN
            -- ★ FIX: aceitar tanto chave com sufixo (liquidar_aposta_v4) quanto sem
            SELECT EXISTS(
              SELECT 1 FROM financial_events
              WHERE aposta_id = p_aposta_id
                AND tipo_evento IN ('PAYOUT','FREEBET_PAYOUT','VOID_REFUND')
                AND bookmaker_id = v_perna.bookmaker_id
                AND (
                  idempotency_key = 'payout_' || p_aposta_id::TEXT || '_perna_' || v_perna.id::TEXT || '_' || v_resultado_anterior
                  OR idempotency_key = 'payout_' || p_aposta_id::TEXT || '_perna_' || v_perna.id::TEXT
                )
            ...
            IF NOT v_payout_event_exists THEN
              INSERT INTO financial_events (
                ...
                'payout_' || p_aposta_id::TEXT || '_perna_' || v_perna.id::TEXT || '_' || v_resultado_anterior,
...
-- ============================================================
-- FIX 2: Trigger BEFORE UPDATE para recalcular pl_consolidado
-- automaticamente em multi-entry multi-currency
-- ============================================================
...
BEGIN
  IF NEW.status <> 'LIQUIDADA' THEN
    RETURN NEW;
  END IF;
  IF OLD.status = 'LIQUIDADA' AND OLD.resultado IS NOT DISTINCT FROM NEW.resultado THEN
    RETURN NEW;
  END IF;

  SELECT COUNT(*) INTO v_perna_count FROM apostas_pernas WHERE aposta_id = NEW.id;
  IF v_perna_count = 0 THEN RETURN NEW; END IF;
  ...
  SELECT
    ...
    COALESCE(cotacao_trabalho_ars, 1) as r_ars,
    COALESCE(cotacao_trabalho_cop, 1) as r_cop
  INTO v_proj FROM projetos WHERE id = NEW.projeto_id;
  IF NOT FOUND THEN RETURN NEW; END IF;

  SELECT COUNT(DISTINCT moeda) INTO v_moedas_distintas FROM apostas_pernas WHERE aposta_id = NEW.id;
  v_is_multi := v_moedas_distintas > 1
    OR EXISTS(SELECT 1 FROM apostas_pernas WHERE aposta_id = NEW.id AND moeda <> v_proj.moeda_consolidacao);
  ...
  FOR v_perna IN
    SELECT moeda, COALESCE(lucro_prejuizo, 0) as lp FROM apostas_pernas WHERE aposta_id = NEW.id
  LOOP
    v_rate_perna := CASE v_perna.moeda
      WHEN 'BRL' THEN 1
      WHEN 'USD' THEN v_proj.r_usd
...
-- ============================================================
-- FIX 3: Reverter saldos inflados de ALAWIN/MY EMPIRE
-- (workspace LABBET ONE)
-- ============================================================
...
INSERT INTO financial_events (
  ...
  (
    ...
    'AJUSTE', 'NORMAL',
    -100, 'MXN',
    'fix_dup_backfill_alawin_2026_04_20',
    'Correção: reverter PAYOUT duplicado pelo backfill multi-entry (aposta excluída ab504dc6)',
    now()
  ),
  (
    ...
    'AJUSTE', 'NORMAL',
    -100, 'USD',
    'fix_dup_backfill_myempire_2026_04_20',
    'Correção: reverter PAYOUT duplicado pelo backfill multi-entry (aposta excluída ab504dc6)',
    now()
  )
