...
-- Fix: Evitar dupla contagem de BONUS_CANCELAMENTO no calendário diário
-- Antes: cl.tipo_transacao='AJUSTE_SALDO' contava todos AJUSTE_SALDO,
-- inclusive os que ja eram contabilizados separadamente em perdas_cancel_daily.
-- Agora: ajustes_daily exclui BONUS_CANCELAMENTO para evitar dupla contagem.
...
BEGIN
  ...
  ajustes_daily AS (
    ...
    WHERE cl.status = 'CONFIRMADO'
      ...
      -- FIX: Excluir BONUS_CANCELAMENTO daqui — já contabilizado em perdas_cancel_daily
      -- (evita dupla contagem que reduzia o lucro do calendário em -valor_perdido)
