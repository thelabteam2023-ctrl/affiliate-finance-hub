-- 1. Corrigir a função liquidar_perna_surebet_v1 para evitar auto-estorno
...
BEGIN
  ...
  -- Buscamos todos os payouts/refunds ativos desta perna e criamos reversões
  -- Adicionada restrição created_at < v_now para não estornar o que acabamos de criar se a função rodar em loop ou trigger
  ...
  WHERE fe.aposta_id = v_surebet_id
    ...
    AND fe.created_at < v_now -- PROTEÇÃO: Não estorna o que for "agora"
    AND (
      fe.idempotency_key LIKE '%perna_' || p_perna_id || '%'
    )
    AND NOT EXISTS (
  ...
  IF p_resultado != 'PENDENTE' AND p_resultado IS NOT NULL THEN
    IF v_has_entries THEN
      FOR v_entry IN 
        SELECT ae.id, ae.bookmaker_id, ae.stake, ae.odd, ae.moeda, COALESCE(ae.fonte_saldo, 'REAL') as fonte_saldo,
               bk.nome as bk_nome
  ...
  -- 4) RECALCULAR TOTAIS DA SUREBET
...
-- 2. Restaurar saldo do Diego: Remover o estorno indevido que anulou o Payout
DELETE FROM public.financial_events 
WHERE id = '40f22aea-fca2-4816-87e6-7e44401a06f8' 
AND aposta_id = 'c192159e-d7b3-46d7-a9ae-c69d01a87713';

-- 3. Sincronizar saldo final
SELECT public.sync_bookmaker_balance_from_ledger('8de2ba2c-011b-49f4-970e-be8637a9b05e');
