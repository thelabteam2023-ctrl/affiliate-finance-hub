...
-- Backfill: gerar eventos AJUSTE para ESTORNOs já criados que ficaram sem efeito no bookmaker
DO $backfill$
DECLARE r record;
BEGIN
  FOR r IN
    SELECT * FROM public.cash_ledger cl
    WHERE cl.descricao LIKE 'ESTORNO:%'
      ...
      AND NOT EXISTS (
        ...
        WHERE fe.idempotency_key = 'ledger_ajuste_' || cl.id
      )
  LOOP
    INSERT INTO public.financial_events (
      ...
      gen_random_uuid(),
      COALESCE(r.destino_bookmaker_id, r.origem_bookmaker_id),
      r.workspace_id, r.user_id,
      'AJUSTE', 'NORMAL',
      CASE WHEN r.ajuste_direcao = 'SAIDA' THEN -1 * ABS(r.valor) ELSE ABS(r.valor) END,
      r.moeda,
      'ledger_ajuste_' || r.id,
      COALESCE(r.descricao, 'Estorno backfill'),
      jsonb_build_object('ledger_id', r.id, 'backfill', true),
      now(), true, 'REAL'::event_scope
    );
    UPDATE public.cash_ledger SET financial_events_generated = true WHERE id = r.id;
  END LOOP;
END
$backfill$;