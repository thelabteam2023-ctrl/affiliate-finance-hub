-- 1) Atualizar a função fn_cash_ledger_generate_financial_events para incluir AJUSTE_SALDO
...
BEGIN
    ...
    -- AJUSTE_MANUAL / AJUSTE_SALDO (Ajuste de Saldo em Vínculos)
    IF NEW.tipo_transacao IN ('AJUSTE_MANUAL', 'AJUSTE_SALDO') AND (NEW.destino_bookmaker_id IS NOT NULL OR NEW.origem_bookmaker_id IS NOT NULL) THEN
        DECLARE
...
-- 2) Regularização Retroativa: AJUSTE_SALDO que foram ignorados mas marcados como processados
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN 
        ...
        FROM cash_ledger cl
        -- Tenta destino primeiro (ENTRADA), depois origem (SAIDA)
        LEFT JOIN bookmakers b ON b.id = COALESCE(cl.destino_bookmaker_id, cl.origem_bookmaker_id)
        WHERE cl.tipo_transacao = 'AJUSTE_SALDO'
          AND cl.status = 'CONFIRMADO'
          AND cl.financial_events_generated = TRUE -- Já processados mas sem evento
          AND NOT EXISTS (SELECT 1 FROM financial_events fe WHERE fe.idempotency_key = 'ledger_ajuste_' || cl.id::TEXT)
    LOOP
        DECLARE
            v_target_id UUID;
            v_valor NUMERIC;
        BEGIN
            IF r.ajuste_direcao = 'ENTRADA' THEN
                v_target_id := r.destino_bookmaker_id;
                v_valor := COALESCE(r.valor_destino, r.valor);
            ELSE
                v_target_id := r.origem_bookmaker_id;
                v_valor := -COALESCE(r.valor_origem, r.valor);
            END IF;
            ...
            IF v_target_id IS NOT NULL THEN
                INSERT INTO financial_events (
                    ...
                    v_target_id, r.workspace_id,
                    'AJUSTE', 'NORMAL', 'AJUSTE_SALDO',
                    v_valor, COALESCE(r.bk_moeda, r.moeda),
                    'ledger_ajuste_' || r.id::TEXT,
                    'Regularização: Ajuste de saldo via Vínculos #' || r.id::TEXT || ' - ' || COALESCE(r.ajuste_motivo, ''),
                    jsonb_build_object('ledger_id', r.id, 'direcao', r.ajuste_direcao, 'motivo', r.ajuste_motivo, 'tipo_transacao', r.tipo_transacao, 'is_regularizacao', true),
                    NOW(), r.user_id
                );
            END IF;
        END;
