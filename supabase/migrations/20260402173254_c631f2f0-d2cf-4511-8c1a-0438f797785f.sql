...
-- ============================================================
-- BROKER ISOLATION: Proteger broker no reprocessamento
-- ============================================================
...
BEGIN
    -- ============================================================
    -- BROKER ISOLATION: NÃO resetar saldo de contas broker
    -- ============================================================
    
    -- Contar broker preservados para auditoria
    SELECT COUNT(*) INTO v_broker_preserved
    FROM bookmakers
    WHERE workspace_id = p_workspace_id AND is_broker_account = true;
    
    -- Reset APENAS contas padrão (não broker)
    UPDATE public.bookmakers
    SET saldo_atual = 0, saldo_freebet = 0, updated_at = NOW()
    WHERE workspace_id = p_workspace_id
      AND is_broker_account = false;
    GET DIAGNOSTICS v_bookmaker_count = ROW_COUNT;

    -- Deletar eventos APENAS de contas padrão (preservar broker)
    DELETE FROM public.financial_events
    WHERE workspace_id = p_workspace_id
      AND bookmaker_id NOT IN (
        SELECT id FROM bookmakers 
        WHERE workspace_id = p_workspace_id AND is_broker_account = true
      );
    GET DIAGNOSTICS v_events_deleted = ROW_COUNT;

    -- Reset flags do ledger APENAS para entradas que não são de broker
    UPDATE public.cash_ledger
    SET financial_events_generated = FALSE,
        balance_processed_at = NULL
    WHERE workspace_id = p_workspace_id
      AND (
        destino_bookmaker_id IS NULL 
        OR destino_bookmaker_id NOT IN (
          SELECT id FROM bookmakers WHERE workspace_id = p_workspace_id AND is_broker_account = true
        )
      )
      AND (
        origem_bookmaker_id IS NULL
        OR origem_bookmaker_id NOT IN (
          SELECT id FROM bookmakers WHERE workspace_id = p_workspace_id AND is_broker_account = true
        )
      );
    ...
    -- Reconciliar APENAS contas padrão (broker já é skipped pela função)
    FOR v_rec IN
        SELECT b.id
    ...
    RETURN jsonb_build_object(
        ...
        'broker_preserved', v_broker_preserved,
