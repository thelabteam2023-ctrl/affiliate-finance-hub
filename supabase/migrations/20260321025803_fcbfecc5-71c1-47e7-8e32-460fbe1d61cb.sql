...
BEGIN
  ...
  FOREACH v_proj IN ARRAY p_projeto_ids LOOP
    ...
    SELECT jsonb_build_object(
      ...
      -- 2. Cashback manual (data_credito is DATE type)
      'cashback', (
        SELECT COALESCE(jsonb_object_agg(moeda, total), '{}'::jsonb)
        FROM (
          SELECT
      ...
      -- 3. Giros grátis confirmados (data_registro is TIMESTAMPTZ)
      'giros', (
        SELECT COALESCE(jsonb_object_agg(moeda, total), '{}'::jsonb)
        FROM (
          SELECT
      ...
      -- 4. Bônus ganhos (exclui FREEBET, credited_at is TIMESTAMPTZ)
      'bonus', (
        SELECT COALESCE(jsonb_object_agg(moeda, total), '{}'::jsonb)
        FROM (
          SELECT
      ...
      -- 5. Perdas operacionais confirmadas (data_registro is TIMESTAMPTZ)
      'perdas', (
        SELECT COALESCE(jsonb_object_agg(moeda, total), '{}'::jsonb)
        FROM (
          SELECT
      ...
      -- 6. Conciliações de vínculo (created_at is TIMESTAMPTZ)
      'conciliacoes', (
        SELECT COALESCE(jsonb_object_agg(moeda, total), '{}'::jsonb)
        FROM (
          SELECT
      ...
      -- 7. Ajustes de saldo (data_transacao is TIMESTAMPTZ)
      'ajustes_saldo', (
        SELECT COALESCE(jsonb_object_agg(moeda, total), '{}'::jsonb)
        FROM (
          SELECT
      ...
      -- 8. Resultado cambial (data_transacao is TIMESTAMPTZ)
      'resultado_cambial', (
        SELECT COALESCE(jsonb_object_agg(moeda, total), '{}'::jsonb)
        FROM (
          SELECT
      ...
      -- 9. Eventos promocionais (data_transacao is TIMESTAMPTZ)
      'promocionais', (
        SELECT COALESCE(jsonb_object_agg(moeda, total), '{}'::jsonb)
        FROM (
          SELECT
      ...
      -- 10. Perdas cancelamento bônus (data_transacao is TIMESTAMPTZ)
      'perdas_cancelamento', (
        SELECT COALESCE(jsonb_object_agg(moeda, total), '{}'::jsonb)
        FROM (
          SELECT
