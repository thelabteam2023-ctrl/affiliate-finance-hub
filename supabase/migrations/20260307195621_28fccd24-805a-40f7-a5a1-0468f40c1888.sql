...
-- Fix: Always update estado_conta when status_final is 'limitada', regardless of marcar_para_saque
CREATE OR REPLACE FUNCTION desvincular_bookmaker_atomico(
  p_bookmaker_id UUID,
  p_projeto_id UUID,
  p_user_id UUID,
  p_workspace_id UUID,
...
BEGIN
  ...
  IF p_saldo_virtual_efetivo > 0 THEN
    INSERT INTO cash_ledger (
      ...
      false, 'FIAT', 'CONFIRMADO', p_projeto_id,
      jsonb_build_object(
  ...
  -- 5. Desvincular bookmaker e atualizar estado_conta
  -- IMPORTANTE: estado_conta preserva o status real para restauração após saque
  UPDATE bookmakers
  SET projeto_id = NULL,
