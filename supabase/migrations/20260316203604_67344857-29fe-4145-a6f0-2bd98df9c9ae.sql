...
-- Fix criar_aposta_atomica (first overload): remove double-counting in validation
-- The saldo_atual already reflects stakes debited via financial_events,
-- so we should NOT also subtract pending stakes from it.
...
BEGIN
  ...
  LOOP
    ...
    -- CORREÇÃO v10.1: Validar contra saldo_atual diretamente
    -- saldo_atual JÁ reflete stakes anteriores debitadas via financial_events
    -- NÃO subtrair apostas pendentes novamente (causava dupla contagem)
    SELECT 
      b.saldo_atual,
      b.moeda,
      b.nome
  ...
  INSERT INTO apostas_unificada (
    ...
    stake_total, lucro_esperado, roi_esperado,
  ...
  LOOP
    ...
    INSERT INTO apostas_pernas (
      ...
      selecao, selecao_livre, odd, stake, moeda,
