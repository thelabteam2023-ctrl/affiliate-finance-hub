-- 1. Melhorar a função de liquidação v4 para garantir o retorno bruto e logs
    ...
    LOOP
        ...
        LOOP
            ...
            IF v_payout_entry > 0 THEN
                -- Log de Auditoria implícito no financial_events
        ...
        IF p_resultado IN ('GREEN', 'MEIO_GREEN', 'VOID', 'MEIO_RED') THEN
            ...
            IF v_payout_total > 0 THEN
                INSERT INTO public.financial_events (
                    ...
                    format('Retorno Aposta Simples (%s) | Stake: %s | Odd: %s', p_resultado, v_aposta.stake, v_aposta.odd),
                    NOW()
                ) ON CONFLICT (idempotency_key) DO NOTHING;
                IF FOUND THEN 
...
-- 2. Garantir re-sincronização global de segurança
...
DECLARE
  v_bk RECORD;
BEGIN
  FOR v_bk IN SELECT id FROM public.bookmakers LOOP
    PERFORM public.sync_bookmaker_balance_from_ledger(v_bk.id);
  END LOOP;
END;
$$;

-- Executar sincronização inicial para corrigir o estado atual
