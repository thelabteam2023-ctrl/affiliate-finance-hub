-- Remove GANHO_CAMBIAL e PERDA_CAMBIAL do cálculo diário do Lucro Operacional do projeto.
-- FX é evento de tesouraria (variação cambial entre pedido e confirmação de saque/depósito),
-- NÃO resultado operacional de aposta. Vive exclusivamente em Indicadores Financeiros / Caixa.
-- Esta RPC alimenta o calendário e o gráfico Evolução do Lucro da Visão Geral.
...
BEGIN
  ...
  -- REMOVIDO: fx_daily (GANHO_CAMBIAL/PERDA_CAMBIAL)
  -- FX é evento de tesouraria, não resultado operacional. Vive em Indicadores Financeiros/Caixa.
  promo_daily AS (
    SELECT
      (cl.data_transacao AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
      CASE
  ...
  all_events AS (
    ...
    -- REMOVIDO: UNION ALL SELECT dia, moeda, val FROM fx_daily
