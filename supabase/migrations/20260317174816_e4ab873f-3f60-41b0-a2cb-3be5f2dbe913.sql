...
CREATE OR REPLACE FUNCTION get_projeto_dashboard_data(p_projeto_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
      ...
      FROM (
        ...
        WHERE status = 'CONFIRMADO'
          ...
          AND (
            ...
            OR (
              (destino_bookmaker_id = ANY(v_bookmaker_ids) OR origem_bookmaker_id = ANY(v_bookmaker_ids))
              AND tipo_transacao IN ('AJUSTE_SALDO', 'GANHO_CAMBIAL', 'PERDA_CAMBIAL', 'FREEBET_CONVERTIDA', 'CREDITO_PROMOCIONAL', 'GIRO_GRATIS_GANHO')
            )
          )
      ) e
    ),
