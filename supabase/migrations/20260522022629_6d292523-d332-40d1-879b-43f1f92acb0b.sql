...
BEGIN
  ...
  -- 1.1 CAPTURAR SNAPSHOT PARA AUDITORIA
  ...
  -- 2. Sincronizar PERNAS (Primeiro coletamos IDs)
  ...
  -- 2.1 Cleanup de Ledger para PERNAS removidas (Importante: rodar antes de mexer nas chaves)
  ...
  -- 2.2 Resetar ordens para evitar colisão temporária (Dirty Trick para chaves Unique sem deferrable)
  UPDATE public.apostas_pernas SET ordem = ordem + 1000 WHERE aposta_id = p_aposta_id;

  -- 2.3 Atualizar pernas existentes e inserir novas
  v_perna_idx := 0;
  FOR v_elem IN SELECT * FROM jsonb_array_elements(p_pernas) LOOP
  ...
  -- 7. FINALIZAR SNAPSHOT DE AUDITORIA
