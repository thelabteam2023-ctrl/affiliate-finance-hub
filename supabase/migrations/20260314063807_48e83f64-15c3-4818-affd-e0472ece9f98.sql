...
-- Fix: Remove fallback clause in get_projeto_dashboard_data that causes double-counting
-- The fallback was: OR (projeto_id_snapshot IS NULL AND destino_bookmaker_id = ANY(v_bookmaker_ids))
-- This captured deposits without projeto_id_snapshot, duplicating virtual deposits.
-- Now only uses projeto_id_snapshot = p_projeto_id (matching the FinancialMetricsPopover behavior)
    ...
    'ledger_extras', (
      ...
      FROM (
        ...
        WHERE tipo_transacao IN (
          ...
          'AJUSTE_SALDO', 'PERDA_OPERACIONAL', 'PERDA_CAMBIAL',
          'GANHO_CAMBIAL', 'BONUS_CANCELAMENTO',
          'EVENTO_PROMOCIONAL', 'CONCILIACAO'
        )
        AND status = 'CONFIRMADO'
        AND (
          projeto_id_snapshot = p_projeto_id
          OR (projeto_id_snapshot IS NULL AND (destino_bookmaker_id = ANY(v_bookmaker_ids) OR origem_bookmaker_id = ANY(v_bookmaker_ids)))
        )
      ) le
    ),
