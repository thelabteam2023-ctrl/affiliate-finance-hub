  ...
  result := jsonb_build_object(
    ...
    'extras_ledger', (
      ...
      FROM (
        SELECT id, tipo_transacao, valor, moeda, data_transacao,
               ...
               origem_bookmaker_id, projeto_id_snapshot
        FROM cash_ledger
        WHERE status = 'CONFIRMADO'
          ...
          AND (
            ...
            OR (
              destino_bookmaker_id = ANY(v_bookmaker_ids)
              AND tipo_transacao IN ('FREEBET_CONVERTIDA', 'CREDITO_PROMOCIONAL', 'GIRO_GRATIS_GANHO')
            )
          )
      ) e
    ),
    ...
    'bonus_cancelamentos', (
      ...
      FROM (
        ...
        FROM cash_ledger
        WHERE ajuste_motivo = 'BONUS_CANCELAMENTO'
          AND ajuste_direcao = 'SAIDA'
