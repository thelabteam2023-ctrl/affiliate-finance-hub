...
-- 1. Função para recalcular saldo de bookmaker baseado no ledger
...
DECLARE
  ...
  v_workspace_id UUID;
BEGIN
  -- Obter saldo anterior e workspace
  SELECT saldo_atual, workspace_id INTO v_saldo_anterior, v_workspace_id
  FROM bookmakers WHERE id = p_bookmaker_id;

  -- Calcular saldo baseado no ledger
  SELECT COALESCE(SUM(
    CASE 
      -- Créditos (depósitos, ganhos)
      WHEN cl.destino_bookmaker_id = p_bookmaker_id AND cl.tipo_transacao IN ('DEPOSITO', 'BONUS_CREDITADO', 'CASHBACK_MANUAL', 'GANHO_CAMBIAL', 'APOSTA_GREEN', 'APOSTA_VOID', 'PERDA_REVERSAO', 'GIRO_GRATIS') THEN
        COALESCE(cl.valor_destino, cl.valor)
      -- Créditos via ajuste de entrada
      WHEN cl.destino_bookmaker_id = p_bookmaker_id AND cl.tipo_transacao IN ('AJUSTE_SALDO', 'AJUSTE_MANUAL') AND cl.ajuste_direcao = 'ENTRADA' THEN
        COALESCE(cl.valor_destino, cl.valor)
      -- Débitos (saques, perdas)
      WHEN cl.origem_bookmaker_id = p_bookmaker_id AND cl.tipo_transacao IN ('SAQUE', 'APOSTA_RED', 'PERDA_CAMBIAL', 'BONUS_ESTORNO', 'CASHBACK_ESTORNO', 'PERDA_OPERACIONAL', 'GIRO_GRATIS_ESTORNO') THEN
        -COALESCE(cl.valor_origem, cl.valor)
      -- Débitos via ajuste de saída
      WHEN cl.origem_bookmaker_id = p_bookmaker_id AND cl.tipo_transacao IN ('AJUSTE_SALDO', 'AJUSTE_MANUAL') AND cl.ajuste_direcao = 'SAIDA' THEN
        -COALESCE(cl.valor_origem, cl.valor)
      -- Transferências
      WHEN cl.destino_bookmaker_id = p_bookmaker_id AND cl.tipo_transacao = 'TRANSFERENCIA' THEN
        COALESCE(cl.valor_destino, cl.valor)
      WHEN cl.origem_bookmaker_id = p_bookmaker_id AND cl.tipo_transacao = 'TRANSFERENCIA' THEN
        -COALESCE(cl.valor_origem, cl.valor)
      ELSE 0
    END
  ), 0)
  ...
  -- Atualizar bookmaker
  UPDATE bookmakers 
  SET saldo_atual = v_saldo_calculado, updated_at = NOW()
  ...
  INSERT INTO bookmaker_balance_audit (
    ...
    v_saldo_calculado,
    'RECALCULO_MANUAL',
    'CORRECAO_BUG_TRIGGER_V3',
    'Recálculo após identificação de processamento duplicado pelo trigger V3'
  );
...
-- 2. Adicionar coluna para rastrear se transação já foi processada
ALTER TABLE cash_ledger 
ADD COLUMN IF NOT EXISTS balance_processed_at TIMESTAMPTZ DEFAULT NULL;

-- 3. Atualizar trigger V3 com proteção contra reprocessamento
...
BEGIN
  ...
  CASE NEW.tipo_transacao
    WHEN 'DEPOSITO', 'BONUS_CREDITADO', 'CASHBACK_MANUAL', 'GANHO_CAMBIAL', 'APOSTA_GREEN', 'APOSTA_VOID', 'PERDA_REVERSAO', 'GIRO_GRATIS' THEN
      v_bookmaker_id := NEW.destino_bookmaker_id;
      v_valor := COALESCE(NEW.valor_destino, NEW.valor);
      v_operacao := 'CREDITO';
    
    WHEN 'SAQUE', 'APOSTA_RED', 'PERDA_CAMBIAL', 'BONUS_ESTORNO', 'CASHBACK_ESTORNO', 'PERDA_OPERACIONAL', 'GIRO_GRATIS_ESTORNO' THEN
...
-- 4. Recriar trigger como BEFORE (para modificar NEW.balance_processed_at)
DROP TRIGGER IF EXISTS tr_cash_ledger_update_bookmaker_balance_v3 ON public.cash_ledger;
