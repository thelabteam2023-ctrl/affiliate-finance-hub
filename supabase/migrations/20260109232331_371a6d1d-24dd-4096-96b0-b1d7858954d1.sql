-- Corrigir as funções para usar valores em MAIÚSCULAS conforme check constraints
...
BEGIN
  ...
  IF v_should_process THEN
    ...
    -- Criar lançamento no cash_ledger (valores MAIÚSCULOS conforme constraints)
    INSERT INTO cash_ledger (
      ...
      NEW.valor_retorno,
      NEW.data_registro,
      'CONFIRMADO',   -- CHECK constraint exige maiúsculas
      'Retorno de Giro Grátis - ' || COALESCE(NEW.observacoes, 'Sem descrição'),
      'BOOKMAKER',    -- CHECK constraint exige maiúsculas
      NEW.bookmaker_id
    )
