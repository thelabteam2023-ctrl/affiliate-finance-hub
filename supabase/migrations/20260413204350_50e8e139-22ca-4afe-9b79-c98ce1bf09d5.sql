...
BEGIN
  ...
  SELECT
    ...
    -- saldo_bonus: APENAS INFORMATIVO para breakdown na UI
    ...
    -- saldo_disponivel: saldo real - stakes NÃO debitadas
    GREATEST(ba.saldo_base - COALESCE(cnd.total_stake, 0), 0) AS saldo_disponivel,
    -- CORREÇÃO CRÍTICA: saldo_operavel = saldo_disponivel + freebet
    -- O bônus JÁ está incluído em saldo_real (via financial_events/trigger),
    -- então NÃO deve ser somado novamente aqui (causava duplicação)
    GREATEST(ba.saldo_base - COALESCE(cnd.total_stake, 0), 0) + COALESCE(ba.saldo_freebet, 0) AS saldo_operavel,
