...
BEGIN
  ...
  -- =====================================================================
  -- CORREÇÃO DOUBLE-COUNTING v14.1:
  -- O saldo_atual JÁ foi debitado pelo trigger fn_financial_events_sync_balance.
  -- Só subtraímos apostas pendentes que AINDA NÃO têm STAKE no ledger.
  -- =====================================================================
  -- Apostas simples pendentes SEM evento de STAKE já processado
  apostas_pendentes_nao_debitadas AS (
    ...
    WHERE au.workspace_id = v_workspace_id
      ...
      -- Excluir apostas que JÁ tiveram stake debitada via financial_events
      AND NOT EXISTS (
  ...
  -- Apostas GLOBAIS pendentes SEM evento de STAKE (para saldo_em_aposta informativo)
  todas_apostas_pendentes_nao_debitadas AS (
    SELECT
      au.bookmaker_id,
  ...
  -- Pernas de surebet pendentes SEM evento de STAKE já processado
  pernas_pendentes_nao_debitadas AS (
    ...
    WHERE au.workspace_id = v_workspace_id
      ...
      AND au.cancelled_at IS NULL
      AND (p_projeto_id IS NULL OR au.projeto_id = p_projeto_id)
      -- Excluir pernas que JÁ tiveram stake debitada
      AND NOT EXISTS (
        SELECT 1 FROM public.financial_events fe
        WHERE fe.aposta_id = au.id
          AND fe.tipo_evento = 'STAKE'
  ...
  combined_todas AS (
    ...
    FROM todas_apostas_pendentes_nao_debitadas tap
    FULL OUTER JOIN (
      SELECT
        ap2.bookmaker_id,
      ...
      WHERE au2.workspace_id = v_workspace_id
        ...
        AND NOT EXISTS (
          SELECT 1 FROM public.financial_events fe
          WHERE fe.aposta_id = au2.id
            AND fe.tipo_evento = 'STAKE'
            AND fe.bookmaker_id = ap2.bookmaker_id
        )
  ...
  SELECT
    ...
    -- saldo_em_aposta: apenas stakes NÃO debitadas (informativo)
