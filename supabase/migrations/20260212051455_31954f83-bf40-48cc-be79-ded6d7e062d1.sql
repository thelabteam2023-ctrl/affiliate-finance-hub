...
-- =============================================================
-- FIX: Remove user_id references from Surebet RPCs
-- financial_events table uses 'created_by', NOT 'user_id'
-- =============================================================

-- 1. FIX editar_perna_surebet_atomica
...
BEGIN
  ...
  IF p_new_bookmaker_id IS NOT NULL AND p_new_bookmaker_id != v_old_bk THEN
    -- A1. Reverse stake on old bookmaker
    ...
    -- A2. Create stake on new bookmaker
    INSERT INTO financial_events (id, bookmaker_id, aposta_id, workspace_id, created_by, tipo_evento, tipo_uso, origem, valor, moeda, idempotency_key, descricao, processed_at)
...
-- 3. FIX deletar_perna_surebet_v1
