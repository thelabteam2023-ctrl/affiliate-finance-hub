-- ============================================================
-- HOTFIX: RPCs de migração de freebets quebradas
-- Causa: chamavam public.calcular_valor_restante_freebet() (não existe).
-- Correção: usar a view canônica v_freebets_disponibilidade (FIFO/ledger).
-- ============================================================
...
BEGIN
  ...
  -- Apostas no projeto de origem que consumiram freebet
  ...
  -- *** CORREÇÃO: usar view canônica FIFO em vez de função inexistente ***
...
-- =============================================================
-- preview_migracao_freebet — mesma correção
-- =============================================================
...
BEGIN
  ...
  -- *** CORREÇÃO: view canônica em vez de função inexistente ***
  SELECT COALESCE(valor_restante, v_freebet.valor) INTO v_valor_restante
  FROM public.v_freebets_disponibilidade
  WHERE id = p_freebet_id;
