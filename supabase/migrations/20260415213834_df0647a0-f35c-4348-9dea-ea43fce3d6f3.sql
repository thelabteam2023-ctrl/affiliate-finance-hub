-- Drop BOTH overloaded versions to resolve PostgREST ambiguity
DROP FUNCTION IF EXISTS public.liquidar_perna_surebet_v1(uuid, uuid, text, text, uuid, text);
...
BEGIN
  ...
  IF v_perna.resultado IS NOT NULL AND v_perna.resultado != 'PENDENTE' THEN
    ...
    IF v_old_payout > 0 THEN
      -- Count existing reversals for monotonic key
      ...
      INSERT INTO financial_events (
        bookmaker_id, workspace_id, event_type, amount, direction,
        balance_type, reference_id, reference_type, idempotency_key, created_by
      ) VALUES (
        v_perna.bookmaker_id,
        COALESCE(p_workspace_id, v_aposta.workspace_id),
        'REVERSAL',
        v_old_payout,
        'OUT',
        CASE WHEN v_actual_fonte_saldo = 'FREEBET' THEN 'FREEBET' ELSE 'CASH' END,
        p_perna_id::text,
        'PERNA_SUREBET',
        v_reversal_key,
        v_aposta.user_id
  ...
  -- 8. Create PAYOUT or VOID_REFUND event
  IF v_payout > 0 THEN
    SELECT count(*) INTO v_count FROM financial_events
    ...
    INSERT INTO financial_events (
      ...
      v_perna.bookmaker_id,
      COALESCE(p_workspace_id, v_aposta.workspace_id),
      CASE WHEN p_resultado = 'VOID' THEN 'VOID_REFUND' ELSE 'PAYOUT' END,
      v_payout,
      'IN',
      CASE WHEN v_actual_fonte_saldo = 'FREEBET' THEN 'FREEBET' ELSE 'CASH' END,
      p_perna_id::text,
  ...
  SELECT count(*) INTO v_pernas_liquidadas FROM apostas_pernas WHERE aposta_id = p_surebet_id AND resultado IS NOT NULL AND resultado != 'PENDENTE';
  
  SELECT 
    bool_and(resultado = 'GREEN') INTO v_all_green
  FROM apostas_pernas WHERE aposta_id = p_surebet_id AND resultado IS NOT NULL AND resultado != 'PENDENTE';
  
  SELECT 
    bool_and(resultado = 'RED') INTO v_all_red
  FROM apostas_pernas WHERE aposta_id = p_surebet_id AND resultado IS NOT NULL AND resultado != 'PENDENTE';
  
  SELECT 
    bool_or(resultado = 'VOID') INTO v_has_void
  FROM apostas_pernas WHERE aposta_id = p_surebet_id;
  
  SELECT 
    bool_or(resultado IN ('MEIO_GREEN', 'MEIO_RED')) INTO v_has_meio
  FROM apostas_pernas WHERE aposta_id = p_surebet_id;
  ...
  IF v_pernas_liquidadas = v_total_pernas THEN
    ...
    ELSE v_resultado_final := 'GREEN'; -- mixed results default
    END IF;
    
    -- Use recalc function if available
    BEGIN
    ...
    -- Ensure resultado is set
    UPDATE apostas_unificada SET
      resultado = v_resultado_final,
      status = 'FINALIZADA',
      updated_at = now()
    WHERE id = p_surebet_id AND (resultado IS NULL OR resultado = 'PENDENTE');
  ELSE
    UPDATE apostas_unificada SET
      status = 'PENDENTE',
      updated_at = now()
