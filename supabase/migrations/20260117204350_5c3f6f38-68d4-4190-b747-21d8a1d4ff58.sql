...
-- Fix recalcular_saldo_bookmaker to use valor_creditado_no_saldo instead of bonus_amount
CREATE OR REPLACE FUNCTION public.recalcular_saldo_bookmaker(p_bookmaker_id uuid)
 RETURNS TABLE(bookmaker_id uuid, nome text, moeda text, saldo_anterior numeric, depositos numeric, saques numeric, transferencias_entrada numeric, transferencias_saida numeric, bonus_creditado numeric, lucro_apostas numeric, cashback numeric, giros_gratis numeric, saldo_calculado numeric, diferenca numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- FIX: Bônus creditados - usar valor_creditado_no_saldo (valor REAL creditado)
  -- Fallback para bonus_amount se valor_creditado_no_saldo não existir
  SELECT COALESCE(SUM(
    COALESCE(pb.valor_creditado_no_saldo, pb.bonus_amount)
  ), 0)
