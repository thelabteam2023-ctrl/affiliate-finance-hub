...
BEGIN
  ...
  SELECT * INTO v_projeto FROM projetos WHERE id = v_link.projeto_id;
  ...
  WHERE projeto_id = v_link.projeto_id AND cancelled_at IS NULL;
  ...
  WHERE au.projeto_id = v_link.projeto_id
    AND au.cancelled_at IS NULL AND au.forma_registro = 'ARBITRAGEM';
  ...
  INTO v_lucro_giros
  FROM giros_gratis
  WHERE projeto_id = v_link.projeto_id AND status = 'CONFIRMADO';

  -- ══════════════════════════════════════════════════════════════
  -- BÔNUS (credited + finalized)
  -- ══════════════════════════════════════════════════════════════
  SELECT COALESCE(SUM(
    CASE
      WHEN COALESCE(b.moeda, 'BRL') = v_moeda_consolidacao THEN COALESCE(b.valor_bonus, 0)
      WHEN COALESCE(b.moeda, 'BRL') IN ('USD','USDT','USDC') AND v_moeda_consolidacao = 'BRL' THEN COALESCE(b.valor_bonus, 0) * v_cotacao
      WHEN COALESCE(b.moeda, 'BRL') = 'BRL' AND v_moeda_consolidacao IN ('USD','USDT','USDC') THEN COALESCE(b.valor_bonus, 0) / v_cotacao
      ELSE COALESCE(b.valor_bonus, 0)
    END
  ), 0)
  INTO v_lucro_bonus
  FROM project_bookmaker_link_bonuses b
  JOIN bookmakers bk ON b.bookmaker_id = bk.id
  WHERE bk.projeto_id = v_link.projeto_id
    AND b.status IN ('credited', 'finalized');

  -- ══════════════════════════════════════════════════════════════
  -- AJUSTES (conciliações via cash_ledger)
  -- ══════════════════════════════════════════════════════════════
  SELECT COALESCE(SUM(
    CASE
      WHEN cl.tipo_transacao IN ('AJUSTE_RECONCILIACAO') THEN
        CASE cl.ajuste_direcao
          WHEN 'CREDITO' THEN 
            CASE
  ...
  FROM (
    ...
    LEFT JOIN LATERAL (
      SELECT COUNT(*) AS pernas_count FROM apostas_pernas ap2 WHERE ap2.aposta_id = au.id
    ) pc ON au.forma_registro = 'ARBITRAGEM'
    WHERE au.projeto_id = v_link.projeto_id
      AND au.cancelled_at IS NULL AND au.status = 'LIQUIDADA'
