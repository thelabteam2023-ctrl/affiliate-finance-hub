...
CREATE FUNCTION public.get_bookmaker_saldos(p_projeto_id UUID DEFAULT NULL)
RETURNS TABLE(
  id UUID,
  nome TEXT,
  parceiro_id UUID,
...
BEGIN
  ...
  -- ONLY deposits/transfers pending reconciliation block the house
  ...
  -- Pending withdrawals: informational only, do NOT block
  saques_pendentes AS (
    SELECT
      cl.origem_bookmaker_id AS bookmaker_id,
      COALESCE(SUM(cl.valor), 0) AS total_saque_pendente
    FROM public.cash_ledger cl
    WHERE cl.workspace_id = v_workspace_id
      AND cl.status IN ('PENDENTE', 'pendente')
  ...
  SELECT
    ...
    (GREATEST(0, ba.saldo_base - COALESCE(ap.total_stake, 0)) + COALESCE(tap.total_stake_real, 0) + GREATEST(0, ba.saldo_freebet))::NUMERIC AS saldo_operavel,
