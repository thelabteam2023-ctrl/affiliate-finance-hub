-- Fase 1: Guarda anti-desvinculação-negativa em desvincular_bookmaker_atomico
DROP FUNCTION IF EXISTS public.desvincular_bookmaker_atomico(uuid, uuid, uuid, uuid, text, numeric, text, boolean, boolean);

CREATE OR REPLACE FUNCTION public.desvincular_bookmaker_atomico(p_bookmaker_id uuid, p_projeto_id uuid, p_user_id uuid, p_workspace_id uuid, p_status_final text, p_saldo_virtual_efetivo numeric, p_moeda text, p_marcar_para_saque boolean DEFAULT false, p_is_investor_account boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- [HARDENING Causa D] Bloqueia desvinculação com saldo negativo (>0.01)
  -- Evita criação de bookmakers órfãos com passivo silencioso.
  IF v_saldo_atual < -0.01 THEN
    INSERT INTO public.financial_debug_log (op, bookmaker_id, old_project_id, event_type, payload)
    VALUES (
      'desvincular_bookmaker_atomico',
      p_bookmaker_id,
      p_projeto_id,
      'NEGATIVE_BALANCE_BLOCK',
      jsonb_build_object(
        'saldo_atual', v_saldo_atual,
        'saldo_bonus', v_saldo_bonus,
    ...
    RETURN jsonb_build_object(
      'success', false,
      'code', 'NEGATIVE_BALANCE_BLOCK',
      'error', format(
        'Desvinculação bloqueada: bookmaker com saldo negativo (%s %s). Reconcilie via AJUSTE_SALDO antes de desvincular.',
        to_char(v_saldo_atual, 'FM999G999G990D00'), p_moeda
      ),
      'saldo_atual', v_saldo_atual,
...
-- Fase 2: View de observabilidade de bookmakers órfãos com saldo
CREATE OR REPLACE VIEW public.vw_bookmakers_orfaos_com_saldo AS
SELECT
  b.id,
  b.workspace_id,
  b.nome,
...
WHERE b.projeto_id IS NULL
  AND ABS(COALESCE(b.saldo_atual, 0)) > 0.01;

GRANT SELECT ON public.vw_bookmakers_orfaos_com_saldo TO authenticated;
GRANT SELECT ON public.vw_bookmakers_orfaos_com_saldo TO service_role;