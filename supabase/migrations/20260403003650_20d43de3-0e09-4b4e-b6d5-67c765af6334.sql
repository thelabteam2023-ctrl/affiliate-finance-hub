CREATE OR REPLACE FUNCTION public.close_project_cycle(
  _ciclo_id uuid, 
  _workspace_id uuid,
  _frontend_metrics jsonb DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Always check pendencies regardless of metrics source
  ...
  -- Use frontend metrics if provided (they include proper currency conversion)
  _use_frontend_metrics := _frontend_metrics IS NOT NULL 
    AND _frontend_metrics ? 'lucro_bruto' 
    AND _frontend_metrics ? 'lucro_liquido';

  IF _use_frontend_metrics THEN
    _qtd_apostas := COALESCE((_frontend_metrics->>'qtd_apostas')::int, 0);
    _volume_apostado := COALESCE((_frontend_metrics->>'volume')::numeric, 0);
    _lucro_apostas := COALESCE((_frontend_metrics->>'lucro_apostas')::numeric, 0);
    _lucro_bruto := COALESCE((_frontend_metrics->>'lucro_bruto')::numeric, 0);
    _lucro_liquido := COALESCE((_frontend_metrics->>'lucro_liquido')::numeric, 0);
    _perdas_confirmadas := COALESCE((_frontend_metrics->>'perdas_confirmadas')::numeric, 0);
    _cashback_total := COALESCE((_frontend_metrics->>'cashback')::numeric, 0);
    _giros_gratis_total := COALESCE((_frontend_metrics->>'giros_gratis')::numeric, 0);
  ELSE
    -- Fallback: calculate without currency conversion (legacy behavior)
    SELECT 
      COALESCE(COUNT(*), 0),
      COALESCE(SUM(COALESCE(stake_total, stake, 0)), 0),
      COALESCE(SUM(COALESCE(lucro_prejuizo, 0)), 0)
    INTO _qtd_apostas, _volume_apostado, _lucro_apostas
    FROM apostas_unificada
    WHERE projeto_id = _ciclo.projeto_id
      AND data_aposta >= _start_utc
      AND data_aposta <= _end_utc
    ...
    SELECT COALESCE(SUM(valor), 0) INTO _cashback_total
    FROM cashback_manual
    WHERE projeto_id = _ciclo.projeto_id
      AND data_credito >= _ciclo.data_inicio
      AND data_credito <= _ciclo.data_fim_prevista;

    SELECT COALESCE(SUM(GREATEST(valor_retorno, 0)), 0) INTO _giros_gratis_total
    FROM giros_gratis
    WHERE projeto_id = _ciclo.projeto_id
      AND status = 'confirmado'
      AND data_registro >= _start_utc
      AND data_registro <= _end_utc;
  ...
  IF _cashback_total > 0 THEN
    _observacoes_finais := _observacoes_finais || format(' | Cashback: R$ %s', ROUND(_cashback_total, 2));
  END IF;

  IF _giros_gratis_total > 0 THEN
    _observacoes_finais := _observacoes_finais || format(' | Giros Grátis: R$ %s', ROUND(_giros_gratis_total, 2));
  END IF;
  ...
  IF _op_projeto.id IS NOT NULL AND _op_projeto.modelo_pagamento IS NOT NULL THEN
    ...
    IF _lucro_base_operador > 0 THEN
      ...
      DELETE FROM pagamentos_propostos WHERE ciclo_id = _ciclo_id;
  ...
  RETURN jsonb_build_object(
    ...
    'metrics', jsonb_build_object(
      ...
      'lucro_apostas', _lucro_apostas,
      'cashback', _cashback_total,
      'giros_gratis', _giros_gratis_total,
      'lucro_bruto', _lucro_bruto,
      'lucro_liquido', _lucro_liquido,
      'roi', _roi,
