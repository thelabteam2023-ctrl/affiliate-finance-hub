-- One-shot deletion of BROKER BÔNUS bypassing the Caixa Operacional protection trigger
DO $$
DECLARE
  _ws uuid := 'aa47a3f3-f679-4601-84c3-645b3085ea47';
  _tbl text; _tmp bigint; _deleted bigint := 0; _remaining text[]; _next text[]; _pass int := 0;
  _tables text[] := ARRAY[
...
BEGIN
  ...
  DELETE FROM public.movimentacoes_indicacao
   WHERE parceiro_id IN (SELECT id FROM public.parceiros WHERE workspace_id=_ws)
      OR origem_parceiro_id IN (SELECT id FROM public.parceiros WHERE workspace_id=_ws);
  DELETE FROM public.projeto_bookmaker_historico
   WHERE parceiro_id IN (SELECT id FROM public.parceiros WHERE workspace_id=_ws);
  UPDATE public.despesas_administrativas SET origem_parceiro_id=NULL
   WHERE origem_parceiro_id IN (SELECT id FROM public.parceiros WHERE workspace_id=_ws);

  _remaining := _tables;
  WHILE array_length(_remaining,1) > 0 AND _pass < 15 LOOP
    _pass := _pass + 1; _next := ARRAY[]::text[];
    FOREACH _tbl IN ARRAY _remaining LOOP
      BEGIN
        EXECUTE format('DELETE FROM public.%I WHERE workspace_id=$1',_tbl) USING _ws;
        GET DIAGNOSTICS _tmp = ROW_COUNT; _deleted := _deleted + _tmp;
      EXCEPTION
    ...
    IF _next = _remaining THEN
      ...
      RAISE EXCEPTION 'Restantes: %', _next;
    END IF;
    _remaining := _next;
  END LOOP;

  BEGIN EXECUTE 'DELETE FROM public.user_roles WHERE workspace_id=$1' USING _ws;
  EXCEPTION WHEN undefined_column OR undefined_table THEN NULL; END;
  DELETE FROM public.workspaces WHERE id=_ws;

  ALTER TABLE public.parceiros ENABLE TRIGGER tr_protect_caixa_operacional;

  RAISE NOTICE 'Workspace BROKER BÔNUS excluído. Linhas removidas: %', _deleted;
END $$;

-- Update the user-facing RPC to also bypass the trigger
CREATE OR REPLACE FUNCTION public.delete_workspace_cascade(
  _workspace_id uuid, _confirm_name text
) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  _ws public.workspaces%ROWTYPE; _caller uuid := auth.uid(); _is_owner boolean;
  _tbl text; _tmp bigint; _deleted bigint := 0; _remaining text[]; _next text[]; _pass int := 0;
  _tables text[] := ARRAY[
    'aposta_edit_audit_logs','bookmaker_balance_audit','bookmaker_stake_reservations',
    'bookmaker_unlinked_acks','bookmaker_workspace_access','bookmaker_grupo_membros',
...
BEGIN
  IF _caller IS NULL THEN RAISE EXCEPTION 'Não autenticado' USING ERRCODE='42501'; END IF;
  ...
  IF _ws.is_active IS TRUE OR _ws.deactivated_at IS NULL THEN
    RAISE EXCEPTION 'Workspace precisa estar desativado antes da exclusão definitiva' USING ERRCODE='22023'; END IF;
  IF _confirm_name IS NULL OR _confirm_name <> _ws.name THEN
    RAISE EXCEPTION 'Nome de confirmação não confere' USING ERRCODE='22023'; END IF;
  SELECT EXISTS(SELECT 1 FROM public.workspace_members
    WHERE workspace_id=_workspace_id AND user_id=_caller AND role='owner' AND is_active=true) INTO _is_owner;
  IF NOT _is_owner THEN
    RAISE EXCEPTION 'Apenas o owner do workspace pode excluir definitivamente' USING ERRCODE='42501'; END IF;
  ...
  BEGIN EXECUTE 'DELETE FROM public.user_roles WHERE workspace_id=$1' USING _workspace_id;
  EXCEPTION WHEN undefined_column OR undefined_table THEN NULL; END;
  DELETE FROM public.workspaces WHERE id=_workspace_id;

  ALTER TABLE public.parceiros ENABLE TRIGGER tr_protect_caixa_operacional;

  RETURN jsonb_build_object('success',true,'workspace_id',_workspace_id,'workspace_name',_ws.name,'rows_deleted',_deleted,'passes',_pass);
END $$;
