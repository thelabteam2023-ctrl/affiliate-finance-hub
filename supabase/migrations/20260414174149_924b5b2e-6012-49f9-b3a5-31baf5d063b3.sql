...
BEGIN
  ...
  IF p_include_financial THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'destino_parceiro_id', cl.destino_parceiro_id, 'destino_conta_bancaria_id', cl.destino_conta_bancaria_id,
      'destino_wallet_id', cl.destino_wallet_id, 'bookmaker_nome', bm.nome,
      'bookmaker_logo_url', bc.logo_url, 'parceiro_nome', par.nome, 'banco_nome', b.nome,
      'wallet_exchange', wc.exchange, 'wallet_network', wc.network, 'wallet_moedas', wc.moeda,
      'projeto_nome', pr.nome, 'coin', cl.coin, 'qtd_coin', cl.qtd_coin, 'cotacao', cl.cotacao,
  ...
  IF p_include_financial THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'base_calculo', pa.base_calculo, 'lucro_base', pa.lucro_base,
      'valor_participacao', pa.valor_participacao, 'data_apuracao', pa.data_apuracao,
      'investidor_nome', inv.nome, 'projeto_nome', pr.nome, 'ciclo_numero', c.numero
    )), '[]'::jsonb) INTO v_section
    FROM participacoes pa
    JOIN investidores inv ON inv.id = pa.investidor_id
    LEFT JOIN projetos pr ON pr.id = pa.projeto_id
    LEFT JOIN ciclos c ON c.id = pa.ciclo_id
  ...
  IF p_include_financial THEN
    ...
    WHERE bk.workspace_id = p_workspace_id AND bk.projeto_id IS NULL
      AND bk.status IN ('ativo', 'aguardando_saque') AND bk.saldo_atual > 0;
  ELSE
    v_section := '[]'::jsonb;
  END IF;
  ...
  IF p_include_financial THEN
    ...
    FROM (
      ...
      LEFT JOIN parceiros par ON par.id = bk.parceiro_id
      WHERE cl.workspace_id = p_workspace_id AND cl.status = 'PENDENTE'
  ...
  -- 15. Propostas de pagamento
  IF p_include_financial THEN
    SELECT COUNT(*)::int INTO v_count
    FROM propostas_pagamento WHERE workspace_id = p_workspace_id AND status = 'PENDENTE';
