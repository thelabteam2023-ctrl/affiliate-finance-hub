...
SELECT 
    ...
    -- Bônus líquido: creditados - cancelados - estornados (exclusão)
    ...
    - COALESCE(( SELECT sum(cl.valor)
           FROM cash_ledger cl
          WHERE (cl.origem_bookmaker_id = b.id OR cl.destino_bookmaker_id = b.id)
            AND cl.tipo_transacao = 'BONUS_ESTORNO'), 0) AS resultado_bonus,
    -- Total: apostas + giros + cashback + bonus líquido
    COALESCE(( SELECT sum(COALESCE(a.pl_consolidado, a.lucro_prejuizo))
           FROM apostas_unificada a
    ...
    - COALESCE(( SELECT sum(cl.valor)
           FROM cash_ledger cl
          WHERE (cl.origem_bookmaker_id = b.id OR cl.destino_bookmaker_id = b.id)
            AND cl.tipo_transacao = 'BONUS_ESTORNO'), 0) AS resultado_operacional_total,
    ( SELECT count(*)
           FROM apostas_unificada a
