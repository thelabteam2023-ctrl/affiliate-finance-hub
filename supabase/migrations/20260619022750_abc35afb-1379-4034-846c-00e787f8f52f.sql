-- =========================================================
-- LAY SUPPORT — Liquidation + Stake Debit (liability based)
-- =========================================================
-- 1) fn_sync_stake_event_v1: para lay debita liability (stake*(odd-1))
-- 2) liquidar_perna_surebet_v1: branches GREEN/MEIO_GREEN/MEIO_RED/RED/VOID para lay com comissão
-- 3) editar_surebet_completa_v3: sem alteração de fluxo (já usa as duas funções acima)
-- Linhas legadas (tipo='back') mantêm comportamento IDÊNTICO.

-- --- 1) STAKE DEBIT --------------------------------------------------------
CREATE OR REPLACE FUNCTION public.fn_sync_stake_event_v1(
  p_entrada_id uuid, p_aposta_id uuid, p_workspace_id uuid,
  p_bookmaker_id uuid, p_stake numeric, p_moeda text,
  p_fonte_saldo text, p_user_id uuid
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_descricao TEXT;
BEGIN
  -- Detecta se a entrada é lay e calcula liability
  ...
  IF v_tipo = 'lay' AND v_odd > 1 THEN
    v_valor_debitar := p_stake * (v_odd - 1);  -- liability
      ...
      LOOP
        ...
        IF v_is_lay THEN
          ...
          IF p_resultado = 'GREEN' THEN
            ...
            v_refund := v_liability; -- libera liability
