-- =============================================
-- RPC: Fechamento de Ciclo com Validação e Auditoria
-- =============================================

-- 1. Criar função de fechamento de ciclo (segura e auditada)
...
BEGIN
  ...
  -- Verificar role do usuário no workspace
  ...
  -- Verificar permissão (apenas owner/admin podem fechar)
  ...
  -- ========== VERIFICAÇÕES DE PENDÊNCIAS ==========
  
  -- Verificar apostas em aberto no período
  ...
  -- Verificar perdas pendentes no período
  ...
  -- Se há pendências, retornar erro com detalhes
  ...
  SELECT 
    _qtd_apostas + COALESCE(COUNT(*), 0),
    _volume_apostado + COALESCE(SUM(stake), 0),
    _lucro_bruto + COALESCE(SUM(lucro_prejuizo), 0)
  INTO _qtd_apostas, _volume_apostado, _lucro_bruto
  FROM apostas_multiplas
  WHERE projeto_id = _ciclo.projeto_id
    AND data_aposta >= _ciclo.data_inicio
    AND data_aposta <= _ciclo.data_fim_prevista
    AND resultado IN ('GREEN', 'RED', 'VOID', 'MEIO_GREEN', 'MEIO_RED');

  -- Adicionar surebets
  SELECT 
    _qtd_apostas + COALESCE(COUNT(*), 0),
    _volume_apostado + COALESCE(SUM(stake_total), 0),
    _lucro_bruto + COALESCE(SUM(lucro_real), 0)
  INTO _qtd_apostas, _volume_apostado, _lucro_bruto
  FROM surebets
  WHERE projeto_id = _ciclo.projeto_id
    AND data_evento >= _ciclo.data_inicio
    AND data_evento <= _ciclo.data_fim_prevista
    AND status = 'LIQUIDADA';
  ...
  -- Excedente (para ciclos por volume)
  _excedente := 0;
  ...
  _observacoes_finais := format(
    ...
    _qtd_apostas, 
    ROUND(_volume_apostado, 2), 
    ROUND(_ticket_medio, 2), 
    ROUND(_roi, 2)
  );
  ...
  INSERT INTO audit_logs (
    ...
    'projeto_ciclo',
    _ciclo_id,
    format('Ciclo %s', _ciclo.numero_ciclo),
    'CLOSE',
    jsonb_build_object(
      'status', _ciclo.status,
      'data_fim_real', _ciclo.data_fim_real
    ),
    jsonb_build_object(
      'status', 'FECHADO',
      'data_fim_real', CURRENT_DATE,
    ...
    jsonb_build_object(
      'projeto_id', _ciclo.projeto_id,
      'numero_ciclo', _ciclo.numero_ciclo,
      'tipo_gatilho', _ciclo.tipo_gatilho,
...
-- 2. Criar função para verificar pendências antes de fechar
CREATE OR REPLACE FUNCTION public.check_cycle_closing_requirements(
  _ciclo_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Calcular preview das métricas
  SELECT 
    COUNT(*) as qtd,
    COALESCE(SUM(stake), 0) as volume,
    COALESCE(SUM(lucro_prejuizo), 0) as lucro
  INTO _metrics
  FROM apostas
  WHERE projeto_id = _ciclo.projeto_id
    AND data_aposta >= _ciclo.data_inicio
