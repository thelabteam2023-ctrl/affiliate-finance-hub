...
BEGIN
  ...
  todas_apostas_pendentes AS (
    SELECT bookmaker_id, SUM(total_stake_real) AS total_stake_real
    FROM (
      SELECT
        au.bookmaker_id,
        COALESCE(SUM(COALESCE(au.stake_real, 0)), 0) AS total_stake_real
      FROM public.apostas_unificada au
      WHERE au.workspace_id = v_workspace_id
        AND au.status = 'PENDENTE'
        AND au.cancelled_at IS NULL
      ...
      SELECT
        ap.bookmaker_id,
        COALESCE(SUM(COALESCE(ap.stake_real, 0)), 0) AS total_stake_real
      FROM public.apostas_pernas ap
      INNER JOIN public.apostas_unificada au ON au.id = ap.aposta_id
      WHERE au.workspace_id = v_workspace_id
        AND au.status = 'PENDENTE'
  ...
  SELECT
    ...
    -- FIX: GREATEST(0, saldo_freebet) impede que freebet negativo reduza saldo_operavel
    (GREATEST(0, ba.saldo_base - COALESCE(ap.total_stake, 0)) + GREATEST(0, ba.saldo_freebet))::NUMERIC AS saldo_operavel,
...
-- FIX 2: Reconciliar saldo_freebet negativo (-1164) da KTO José Roberto
-- Evento FREEBET_CREDIT para neutralizar débitos fantasma de auto-heal
INSERT INTO public.financial_events (
  tipo_evento, tipo_uso, valor, bookmaker_id, workspace_id,
  descricao, idempotency_key
)
SELECT 
  'FREEBET_CREDIT', 'FREEBET', 1164.00,
  '6fd8f4dc-e1be-47ff-a4e9-7b14fad605ea',
  'feee9758-a7f4-474c-b2b1-679b66ec1cd9',
  'Reconciliação: neutralização de saldo_freebet negativo (-1164) causado por FREEBET_STAKE sem crédito prévio',
  'reconcile_freebet_kto_jose_roberto_20260328'
WHERE NOT EXISTS (
  SELECT 1 FROM public.financial_events 
  WHERE idempotency_key = 'reconcile_freebet_kto_jose_roberto_20260328'
)
