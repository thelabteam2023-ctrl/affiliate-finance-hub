...
BEGIN
  ...
  IF p_include_partner THEN
    ...
    -- 8. Parceiros sem origem histórica registrada (EXCLUDE caixa operacional)
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', par.id,
      'nome', par.nome,
      'cpf', par.cpf,
  ...
  -- 9. Saques pendentes (NOW WITH bookmaker_logo_url)
  IF p_include_financial THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', cl.id,
      'valor', cl.valor,
  ...
  IF p_include_financial THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'ciclo_id', pa.ciclo_id,
      'investidor_id', pa.investidor_id,
      'percentual_aplicado', pa.percentual_aplicado,
      'base_calculo', pa.base_calculo,
      'lucro_base', pa.lucro_base,
      'valor_participacao', pa.valor_participacao,
  ...
  -- 13. Casas desvinculadas (no projeto, status ativo, com saldo > 0)
  IF p_include_financial THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', bk.id,
      'nome', bk.nome,
      'saldo_atual', bk.saldo_atual,
      'moeda', bk.moeda,
      'parceiro_nome', par.nome,
      'status', bk.status,
  ...
  IF p_include_financial THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'bookmaker_nome', sub.bookmaker_nome,
      'bookmaker_logo_url', sub.bookmaker_logo_url,
      'moeda', sub.moeda,
      'saldo_atual', sub.saldo_atual,
      'projeto_id', sub.projeto_id,
      'projeto_nome', sub.projeto_nome,
      'parceiro_nome', sub.parceiro_nome,
      'qtd_transacoes_pendentes', sub.qtd_transacoes_pendentes,
      'valor_total_pendente', sub.valor_total_pendente
    )), '[]'::jsonb) INTO v_section
    FROM (
      SELECT
        bk.id as bookmaker_id,
        ...
        COALESCE(SUM(cl.valor), 0) as valor_total_pendente
      FROM cash_ledger cl
  ...
  -- 15. Propostas de pagamento pendentes (count only)
  IF p_include_financial THEN
  ...
  ELSE
    v_result := v_result || jsonb_build_object('propostas_pagamento_count', 0);
  END IF;
