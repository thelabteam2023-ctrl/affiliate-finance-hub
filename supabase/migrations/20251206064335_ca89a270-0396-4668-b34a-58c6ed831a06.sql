...
-- Corrigir view v_painel_operacional para excluir bookmakers que já têm saque pendente de confirmação
...
SELECT 
  ...
  status_anterior
FROM (
  -- Saques pendentes de PROCESSAMENTO (bookmakers AGUARDANDO_SAQUE que ainda NÃO têm transação SAQUE pendente)
  SELECT 
  ...
  WHERE b.status = 'AGUARDANDO_SAQUE' 
    AND b.saldo_atual > 0
    -- Excluir bookmakers que já têm transação SAQUE pendente de confirmação no cash_ledger
    AND NOT EXISTS (
