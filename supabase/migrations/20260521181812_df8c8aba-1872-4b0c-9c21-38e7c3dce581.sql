-- DROP e recriação com schema corrigido
...
BEGIN
  ...
  FOR v_perna_json IN SELECT * FROM jsonb_array_elements(p_pernas) LOOP
    ...
    -- (compatibilidade com componentes que leem apostas_pernas diretamente)
    ...
    FOR v_entrada_json IN 
      SELECT * FROM jsonb_array_elements(p_entradas) 
  ...
  FOR v_entrada_json IN SELECT * FROM jsonb_array_elements(p_entradas) LOOP
    ...
    -- Inserir a entrada (sem workspace_id nem aposta_id, apenas perna_id)
    ...
    -- Sincronizar evento de STAKE no Ledger via RPC idempotente
    -- Padrão já estabelecido pela editar_surebet_completa_v3
...
COMMENT ON FUNCTION public.criar_surebet_atomica_v3 IS 'Versão 1:N da criação de surebet. Suporta múltiplas entradas por perna e utiliza a nova estrutura de tabelas apostas_pernas e apostas_perna_entradas. Sincroniza eventos de stake via fn_sync_stake_event_v1 (padrão idempotente).';
