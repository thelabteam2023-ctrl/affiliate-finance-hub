...
BEGIN
  ...
  v_direcao_oposta := CASE v_tx.tipo_transacao
    -- Entradas em bookmaker => espelho é SAIDA
    ...
    -- Saídas de bookmaker/caixa => espelho é ENTRADA
    ...
    -- Estornos: invertem o sinal do estorno original
    ...
    -- Ajustes preservam direção oposta explícita
  ...
  -- Fallback: se direção ainda nula, deduzir por origem/destino
  IF v_direcao_oposta IS NULL THEN
    ...
    ELSIF v_tx.destino_bookmaker_id IS NOT NULL
      ...
      -- Original era ENTRADA no destino → espelho é SAIDA
