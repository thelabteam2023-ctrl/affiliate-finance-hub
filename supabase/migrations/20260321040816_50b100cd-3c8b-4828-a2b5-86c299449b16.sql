...
BEGIN
  -- Fetch project consolidation currency and working rate
  SELECT p.moeda_consolidacao, COALESCE(p.cotacao_trabalho, 5.0)
  ...
  -- With multi-currency conversion using project consolidation
  -- ══════════════════════════════════════════════════════════════
  SELECT
    ...
    COALESCE(SUM(CASE 
      WHEN forma_registro != 'ARBITRAGEM' THEN 
        CASE
          ...
          WHEN COALESCE(moeda_operacao, 'BRL') IN ('USD', 'USDT', 'USDC') AND v_moeda_consolidacao = 'BRL' THEN COALESCE(stake, 0) * v_cotacao_trabalho
          WHEN COALESCE(moeda_operacao, 'BRL') = 'BRL' AND v_moeda_consolidacao IN ('USD', 'USDT', 'USDC') THEN COALESCE(stake, 0) / v_cotacao_trabalho
          WHEN COALESCE(moeda_operacao, 'BRL') IN ('EUR') AND v_moeda_consolidacao = 'BRL' THEN COALESCE(stake, 0) * v_cotacao_trabalho * 1.08
          ELSE COALESCE(stake, 0)
        END
      ELSE 0
    END), 0),
    -- Lucro with currency conversion (priority: pl_consolidado > brl_referencia > convert)
    COALESCE(SUM(
      CASE
        ...
        WHEN COALESCE(moeda_operacao, 'BRL') IN ('USD', 'USDT', 'USDC') AND v_moeda_consolidacao = 'BRL' THEN COALESCE(lucro_prejuizo, 0) * v_cotacao_trabalho
        WHEN COALESCE(moeda_operacao, 'BRL') = 'BRL' AND v_moeda_consolidacao IN ('USD', 'USDT', 'USDC') THEN COALESCE(lucro_prejuizo, 0) / v_cotacao_trabalho
        WHEN COALESCE(moeda_operacao, 'BRL') IN ('EUR') AND v_moeda_consolidacao = 'BRL' THEN COALESCE(lucro_prejuizo, 0) * v_cotacao_trabalho * 1.08
  ...
  -- With multi-currency conversion for stakes
  -- ══════════════════════════════════════════════════════════════
  SELECT
    COUNT(*),
    ...
    COALESCE(SUM(
      CASE
        ...
        WHEN ap.moeda IN ('USD', 'USDT', 'USDC') AND v_moeda_consolidacao = 'BRL' THEN ap.stake * v_cotacao_trabalho
        WHEN ap.moeda = 'BRL' AND v_moeda_consolidacao IN ('USD', 'USDT', 'USDC') THEN ap.stake / v_cotacao_trabalho
        ELSE ap.stake
      END
    ), 0)
  ...
  -- ══════════════════════════════════════════════════════════════
  -- CASHBACK (only when no strategy filter)
  -- ══════════════════════════════════════════════════════════════
  IF p_estrategia IS NULL THEN
    SELECT COALESCE(SUM(
      CASE 
        ...
        WHEN COALESCE(moeda_operacao, 'BRL') IN ('USD', 'USDT', 'USDC') AND v_moeda_consolidacao = 'BRL' THEN COALESCE(valor, 0) * v_cotacao_trabalho
  ...
  -- With multi-currency conversion
  -- ══════════════════════════════════════════════════════════════
    ...
    SELECT 
      ...
      SUM(
        CASE
          ...
          WHEN COALESCE(au.moeda_operacao, 'BRL') IN ('USD', 'USDT', 'USDC') AND v_moeda_consolidacao = 'BRL' THEN COALESCE(au.lucro_prejuizo, 0) * v_cotacao_trabalho
          WHEN COALESCE(au.moeda_operacao, 'BRL') = 'BRL' AND v_moeda_consolidacao IN ('USD', 'USDT', 'USDC') THEN COALESCE(au.lucro_prejuizo, 0) / v_cotacao_trabalho
