...
BEGIN
  ...
  -- Skip if already linked (direct project change)
  ...
  -- STEP 1: Adopt orphan deposits/withdrawals (NULL snapshot, created AFTER last SV)
  ...
  -- STEP 2: Clear previous cycle deposits that may have this project's snapshot
  -- (handles re-linking to same project scenario: A→B→A)
  -- Uses <= to ensure events created at the same timestamp as SV are also cleared
