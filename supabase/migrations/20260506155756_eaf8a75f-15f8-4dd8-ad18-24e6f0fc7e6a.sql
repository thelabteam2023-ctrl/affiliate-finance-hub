-- ATUALIZAR A FUNÇÃO DO GATILHO DE CONSOLIDAÇÃO
...
BEGIN
  ...
  IF v_has_entries THEN
      ...
      BEGIN
        CASE v_entry.resultado
          ...
          WHEN 'VOID' THEN
            v_entry_lucro := 0;
          ELSE 
            -- PENDING: Se a aposta não está liquidada, não subtraímos o stake do lucro consolidado
            -- Isso evita o "lucro negativo" enquanto a operação está pendente.
            v_entry_lucro := 0;
        END CASE;
    ...
    END LOOP;
  ELSE
    -- CAMINHO LEGADO: Iterar pelas pernas
    FOR v_entry IN
    ...
    LOOP
      ...
      -- No legado, se a perna for nula/pending, tratamos como 0 se desejado
  ...
  ELSE
    -- Se for moeda única, manter o lucro_prejuizo original mas garantir que não é sobrescrito erroneamente
    -- Exceto se estivermos em um contexto de Surebet onde queremos o valor calculado.
...
-- DISPARAR RECALCULO PARA APOSTAS PENDENTES
