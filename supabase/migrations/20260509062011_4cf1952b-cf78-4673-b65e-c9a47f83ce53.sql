-- 1. Corrigir editar_perna_surebet_atomica para evitar dupla reversão
CREATE OR REPLACE FUNCTION public.editar_perna_surebet_atomica(
    p_perna_id uuid,
    p_new_stake numeric DEFAULT NULL::numeric,
    p_new_odd numeric DEFAULT NULL::numeric,
    p_new_bookmaker_id uuid DEFAULT NULL::uuid,
    p_new_selecao text DEFAULT NULL::text,
    p_new_selecao_livre text DEFAULT NULL::text
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_eff_stake > v_old_stake THEN
    ...
    IF v_saldo_atual < v_stake_increase THEN
      RETURN jsonb_build_object('success', false, 'error', 'Saldo insuficiente para aumento de stake');
    END IF;
  END IF;
  ...
  ELSE
    ...
    IF v_stake_diff != 0 THEN
      -- Usa tipo_uso da perna para afetar saldo correto
      INSERT INTO financial_events (bookmaker_id, aposta_id, workspace_id, created_by, tipo_evento, tipo_uso, origem, valor, moeda, idempotency_key, descricao)
...
-- 2. Corrigir editar_surebet_completa_v1 para suportar novas pernas e sincronizar totais
