...
BEGIN
  ...
  IF NOT (_is_owner OR _is_system_owner) THEN
    RAISE EXCEPTION 'Apenas o owner do workspace ou administrador global pode excluir definitivamente' USING ERRCODE = '42501';
  END IF;
    ...
    FOREACH _tbl IN ARRAY _remaining LOOP
      ...
      EXCEPTION
        WHEN undefined_table OR undefined_column THEN
          NULL;
        WHEN OTHERS THEN
          _next := array_append(_next, _tbl);
      END;
    END LOOP;

    IF array_length(_next, 1) IS NULL THEN
      EXIT;
    END IF;
...
REVOKE ALL ON FUNCTION public.admin_set_workspace_active(uuid, boolean, text) FROM PUBLIC;
