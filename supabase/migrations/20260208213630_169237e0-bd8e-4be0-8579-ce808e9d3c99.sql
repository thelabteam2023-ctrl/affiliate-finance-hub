...
-- Fix close_project_cycle: timezone operacional + inclusão de cashback/giros grátis
...
BEGIN
  ...
  -- CRÍTICO: Converter datas do ciclo para timezone operacional São Paulo
  -- Início do dia em SP = 03:00 UTC, Fim do dia em SP = 02:59:59 UTC do dia seguinte
  ...
  -- Verificar pendências usando timezone correto
  ...
  -- Apostas liquidadas com timezone correto
  SELECT 
    COALESCE(COUNT(*), 0),
  ...
  -- NOVO: Cashback manual no período (data_credito é date, não timestamp)
  ...
  -- NOVO: Giros grátis confirmados no período (timestamp, usar timezone)
  ...
  -- Fórmula canônica: LUCRO = APOSTAS + CASHBACK + GIROS - PERDAS
