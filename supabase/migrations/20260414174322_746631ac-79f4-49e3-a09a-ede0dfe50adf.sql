CREATE OR REPLACE FUNCTION public.get_central_operacoes_data(
  p_workspace_id uuid, p_user_id uuid, p_is_operator boolean DEFAULT false,
  p_include_financial boolean DEFAULT true, p_include_partner boolean DEFAULT true
...
BEGIN
  ...
  IF p_include_partner THEN
    ...
    )), '[]'::jsonb) INTO v_section FROM computed c WHERE c.qtd_parceiros >= c.meta_parceiros AND c.bonus_pagos = 0;
    v_result := v_result || jsonb_build_object('bonus_pendentes', v_section);

    -- 6. Comissões pendentes
    SELECT COALESCE(jsonb_agg(jsonb_build_object('parceriaId', p.id, 'parceiroNome', par.nome,
      'indicadorNome', ir.nome, 'valorComissao', p.valor_comissao_indicador
    )), '[]'::jsonb) INTO v_section
    FROM parcerias p JOIN parceiros par ON par.id = p.parceiro_id LEFT JOIN indicadores_referral ir ON ir.id = p.indicacao_id
    WHERE p.workspace_id = p_workspace_id AND p.status IN ('ATIVA', 'EM_ENCERRAMENTO') AND p.indicacao_id IS NOT NULL
      AND COALESCE(p.valor_comissao_indicador, 0) > 0 AND COALESCE(p.comissao_paga, false) = false
      AND NOT EXISTS (SELECT 1 FROM movimentacoes_indicacao m WHERE m.parceria_id = p.id AND m.tipo IN ('COMISSAO', 'COMISSAO_INDICADOR') AND m.status = 'CONFIRMADO');
    v_result := v_result || jsonb_build_object('comissoes_pendentes', v_section);

    -- 7. Parcerias encerramento
    SELECT COALESCE(jsonb_agg(jsonb_build_object('id', p.id, 'parceiro_id', p.parceiro_id, 'parceiroNome', par.nome,
      'diasRestantes', COALESCE((p.data_fim_prevista::date - v_hoje), 999), 'dataFim', p.data_fim_prevista, 'dataInicio', p.data_inicio,
      'duracaoDias', p.duracao_dias, 'valor_parceiro', p.valor_parceiro, 'valor_indicador', COALESCE(p.valor_comissao_indicador, 0),
      'valor_fornecedor', COALESCE(p.valor_fornecedor, 0), 'origem_tipo', COALESCE(p.origem_tipo, 'INDICADOR'),
      'fornecedor_id', p.fornecedor_id, 'indicacao_id', p.indicacao_id, 'elegivel_renovacao', COALESCE(p.elegivel_renovacao, false),
      'observacoes', p.observacoes, 'status', p.status
    )), '[]'::jsonb) INTO v_section
    FROM parcerias p JOIN parceiros par ON par.id = p.parceiro_id
    WHERE p.workspace_id = p_workspace_id AND p.status IN ('ATIVA', 'EM_ENCERRAMENTO') AND p.data_fim_prevista IS NOT NULL AND (p.data_fim_prevista::date - v_hoje) <= 10;
    v_result := v_result || jsonb_build_object('parcerias_encerramento', v_section);

    -- 8. Parceiros sem origem
    SELECT COALESCE(jsonb_agg(jsonb_build_object('id', par.id, 'nome', par.nome, 'cpf', par.cpf, 'createdAt', par.created_at
  ...
  -- 15. Propostas count (table may not exist, use 0 as fallback)
