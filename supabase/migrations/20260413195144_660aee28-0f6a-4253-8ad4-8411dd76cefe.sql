...
BEGIN
  ...
  SELECT
    ...
    -- saldo_disponivel: saldo real - stakes NÃO debitadas (saques pendentes são APENAS informativos, não bloqueiam)
    GREATEST(ba.saldo_base - COALESCE(cnd.total_stake, 0), 0) AS saldo_disponivel,
    -- saldo_operavel: saldo disponível + freebet + bonus
    GREATEST(ba.saldo_base - COALESCE(cnd.total_stake, 0), 0) + COALESCE(ba.saldo_freebet, 0) + COALESCE(bc2.total_bonus, 0) AS saldo_operavel,
