-- Update v_custos_aquisicao to include missing columns needed by the RPC
CREATE OR REPLACE VIEW public.v_custos_aquisicao AS
 SELECT p.user_id,
    ...
    p.workspace_id,
    p.comissao_paga,
    p.valor_comissao_indicador AS valor_comissao
  ...
  WHERE (((p.workspace_id = public.get_current_workspace()) OR ((p.workspace_id IS NULL) AND (p.user_id = auth.uid()))) AND (p.pagamento_dispensado = false));

-- Fix the get_central_operacoes_data RPC
...
DECLARE
  ...
  v_count int;
BEGIN
  -- Set local session variable for the workspace so views that use get_current_workspace() can function correctly
  PERFORM set_config('request.headers', jsonb_build_object('x-workspace-id', p_workspace_id)::text, true);
  ...
  IF p_include_partner THEN
    ...
    WITH forn_pagos AS (
      ...
      WHERE tipo = 'PAGTO_FORNECEDOR' AND status = 'CONFIRMADO' AND workspace_id = p_workspace_id 
    ...
    WITH indicador_counts AS (
      ...
      FROM movimentacoes_indicacao
      WHERE tipo = 'BONUS_INDICADOR' AND status = 'CONFIRMADO' AND workspace_id = p_workspace_id 
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'ciclosPendentes', (c.qtd_parceiros / NULLIF(c.meta_parceiros, 0)) - c.bonus_pagos,
      'totalBonusPendente', ((c.qtd_parceiros / NULLIF(c.meta_parceiros, 0)) - c.bonus_pagos) * c.valor_bonus
    )), '[]'::jsonb) INTO v_section 
    FROM computed c
    WHERE (c.qtd_parceiros / NULLIF(c.meta_parceiros, 0)) > c.bonus_pagos;
    v_result := v_result || jsonb_build_object('bonus_pendentes', v_section);

    -- 6. Comissões pendentes
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'parceriaId', vca.parceria_id, 
      'parceiroNome', vca.parceiro_nome,
      'indicadorId', vca.indicador_id, 
      'indicadorNome', vca.indicador_nome, 
      'valorComissao', vca.valor_comissao
    )), '[]'::jsonb) INTO v_section 
    FROM v_custos_aquisicao vca
    WHERE vca.workspace_id = p_workspace_id 
      AND vca.comissao_paga = false 
      AND vca.valor_comissao > 0;
    ...
    WHERE p.workspace_id = p_workspace_id 
      AND p.status IN ('ATIVA', 'EM_ENCERRAMENTO')
      AND (p.data_fim_prevista - v_hoje) <= 15;
    ...
    WHERE par.workspace_id = p_workspace_id 
      ...
      AND NOT EXISTS (
        ...
        WHERE p.parceiro_id = par.id 
          AND (p.indicacao_id IS NOT NULL OR p.fornecedor_id IS NOT NULL OR p.origem_tipo IS NOT NULL)
      );
    v_result := v_result || jsonb_build_object('parceiros_sem_parceria', v_section);
  ELSE
    v_result := v_result || jsonb_build_object('pagamentos_parceiros', '[]'::jsonb) 
  ...
  IF p_include_financial THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', cl.id, 'valor', cl.valor, 'moeda', cl.moeda, 'data_transacao', cl.data_transacao,
      'descricao', cl.descricao, 'origem_bookmaker_id', cl.origem_bookmaker_id, 
  ...
  IF p_include_partner THEN
    ...
    WHERE t.notificado = false AND t.workspace_id = p_workspace_id;
  ELSE 
    v_section := '[]'::jsonb; 
  END IF;
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', po.id, 'operador_id', po.operador_id, 'operador_nome', op.nome,
    'tipo_pagamento', po.tipo_pagamento, 'valor', po.valor, 
    'data_pagamento', po.data_pagamento, 'projeto_id', po.projeto_id, 'projeto_nome', pr.nome
  )), '[]'::jsonb) INTO v_section 
  ...
  IF p_include_financial THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', pa.id, 'projeto_id', pa.projeto_id, 'ciclo_id', pa.ciclo_id,
      'investidor_id', pa.investidor_id, 'percentual_aplicado', pa.percentual_aplicado, 
      'base_calculo', pa.base_calculo,
      'lucro_base', pa.lucro_base, 'valor_participacao', pa.valor_participacao, 
      'data_apuracao', pa.data_apuracao,
      'investidor_nome', inv.nome, 'projeto_nome', pr.nome, 'ciclo_numero', c.numero_ciclo
    )), '[]'::jsonb) INTO v_section
    FROM participacao_ciclos pa 
    JOIN investidores inv ON inv.id = pa.investidor_id
    LEFT JOIN projetos pr ON pr.id = pa.projeto_id 
    LEFT JOIN projeto_ciclos c ON c.id = pa.ciclo_id
  ...
  IF p_include_financial THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'id', bk.id, 'nome', bk.nome, 'saldo_atual', bk.saldo_atual, 'moeda', bk.moeda,
      'parceiro_nome', par.nome, 'status', bk.status, 'logo_url', bc.logo_url, 
    ...
    WHERE bk.workspace_id = p_workspace_id 
      AND bk.projeto_id IS NULL 
      AND bk.status IN ('ativo', 'aguardando_saque') 
      AND bk.saldo_atual > 0;
  ELSE 
    v_section := '[]'::jsonb; 
  END IF;
  ...
  IF p_include_financial THEN
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
      'bookmaker_id', sub.bookmaker_id, 'bookmaker_nome', sub.bookmaker_nome,
      'bookmaker_logo_url', sub.bookmaker_logo_url, 'moeda', sub.moeda, 
      'saldo_atual', sub.saldo_atual, 'projeto_id', sub.projeto_id,
      'projeto_nome', sub.projeto_nome, 'parceiro_nome', sub.parceiro_nome,
      'qtd_transacoes_pendentes', sub.qtd_transacoes_pendentes, 
      'valor_total_pendente', sub.valor_total_pendente
    )), '[]'::jsonb) INTO v_section FROM (
      SELECT bk.id as bookmaker_id, bk.nome as bookmaker_nome, bc.logo_url as bookmaker_logo_url, 
        bk.moeda, bk.saldo_atual, bk.projeto_id, pr.nome as projeto_nome, par.nome as parceiro_nome,
        COUNT(cl.id)::int as qtd_transacoes_pendentes, COALESCE(SUM(cl.valor), 0) as valor_total_pendente
      FROM cash_ledger cl 
      JOIN bookmakers bk ON bk.id = COALESCE(cl.destino_bookmaker_id, cl.origem_bookmaker_id)
      LEFT JOIN bookmakers_catalogo bc ON bc.id = bk.bookmaker_catalogo_id 
      LEFT JOIN projetos pr ON pr.id = bk.projeto_id
      LEFT JOIN parceiros par ON par.id = bk.parceiro_id
      WHERE cl.workspace_id = p_workspace_id 
        AND cl.status = 'PENDENTE' 
