-- 1. Expand CHECK constraint to include new valid event types
ALTER TABLE financial_events DROP CONSTRAINT financial_events_tipo_evento_check;
...
ALTER TABLE financial_events ADD CONSTRAINT financial_events_tipo_evento_check
  CHECK (tipo_evento = ANY (ARRAY[
    ...
    'BONUS_ESTORNO','TRANSFERENCIA_SAIDA','TRANSFERENCIA_ENTRADA',
    'PERDA_OPERACIONAL','PERDA_REVERSAO'
  ]));

-- 2. Fix trigger function: AJUSTE_MANUAL → AJUSTE (already in constraint)
...
BEGIN
  ...
  CASE NEW.tipo_transacao
    ...
    -- === INVESTMENT OPERATIONS (no financial event) ===
