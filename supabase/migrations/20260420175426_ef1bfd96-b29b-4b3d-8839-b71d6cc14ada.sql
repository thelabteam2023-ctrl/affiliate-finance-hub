-- ============================================================
-- Correção: reliquidar_aposta_v6 + atualizar_aposta_liquidada_atomica_v2
-- Suporte a apostas simples com múltiplas entradas (apostas_pernas)
-- ============================================================

-- 1) Substituir reliquidar_aposta_v6 para iterar por apostas_pernas quando existirem
...
BEGIN
  ...
  IF v_has_pernas THEN
    -- ============== MULTI-ENTRY PATH ==============
    -- Iterar por cada perna e aplicar ajuste na MOEDA NATIVA da casa
    FOR v_perna IN
      SELECT * FROM apostas_pernas WHERE aposta_id = p_aposta_id ORDER BY ordem
    LOOP
      v_is_freebet := COALESCE(v_perna.fonte_saldo, 'REAL') = 'FREEBET';
      ...
      SET
        ...
        lucro_prejuizo = CASE p_novo_resultado
          WHEN 'GREEN' THEN v_stake * (v_odd - 1)
          WHEN 'MEIO_GREEN' THEN v_stake * (v_odd - 1) / 2
          WHEN 'VOID' THEN 0
          WHEN 'MEIO_RED' THEN CASE WHEN v_is_freebet THEN 0 ELSE -v_stake / 2 END
      ...
      IF v_perna.bookmaker_id IS NOT NULL AND v_diferenca <> 0 THEN
        ...
        IF v_evento_existente IS NULL THEN
          INSERT INTO financial_events (
            ...
            format('Reliquidação multi-entry perna %s (%s -> %s)', v_perna.ordem, COALESCE(v_resultado_anterior,'NULL'), p_novo_resultado)
          );
        END IF;
      END IF;
    END LOOP;

    -- Para multi-entry, lucro_prejuizo da aposta-mãe é a soma dos PLs (na moeda da operação ou em diferentes moedas)
    -- Usar p_lucro_prejuizo se fornecido, senão soma das pernas (em moeda nativa - apenas indicativo)
