-- =========================================================================
-- Fix: usar colunas reais de financial_events (sem perna_id/projeto_id/
-- balance_type, com reversed_event_id, descricao, metadata).
-- =========================================================================
...
BEGIN
  ...
  -- Iterar pernas; perna_id é rastreado via metadata->>'perna_id'
  FOR v_perna IN
    ...
    ORDER BY ap.ordem
  LOOP
    -- Buscar PAYOUT/VOID_REFUND ativo desta perna (via metadata) e
    -- não revertido (via reversed_event_id).
    SELECT fe.id, fe.valor, fe.moeda, fe.created_at, fe.tipo_evento
    INTO v_payout_event
    FROM financial_events fe
    WHERE fe.aposta_id = v_aposta.id
      AND fe.bookmaker_id = v_perna.bookmaker_id
      AND fe.tipo_evento IN ('PAYOUT','VOID_REFUND')
      AND COALESCE(fe.metadata->>'perna_id', v_perna.perna_id::text) = v_perna.perna_id::text
      AND NOT EXISTS (
...
BEGIN
  ...
  FOR v_perna IN
    ...
    ORDER BY ap.ordem
  LOOP
    SELECT fe.id, fe.valor, fe.moeda, fe.tipo_evento, fe.metadata
    INTO v_payout_event
    FROM financial_events fe
    WHERE fe.aposta_id = v_aposta.id
      AND fe.bookmaker_id = v_perna.bookmaker_id
    ...
    IF v_payout_event.id IS NOT NULL AND v_payout_event.valor > 0 THEN
      ...
      INSERT INTO financial_events (
        workspace_id, bookmaker_id, aposta_id,
        tipo_evento, valor, moeda, event_scope,
        idempotency_key, reversed_event_id, descricao, created_by, metadata
      ) VALUES (
        v_aposta.workspace_id, v_perna.bookmaker_id, v_aposta.id,
        'REVERSAL', -v_payout_event.valor, v_payout_event.moeda, 'REAL',
        v_idem_key, v_payout_event.id,
        'Reabertura de surebet liquidada — estorno do ' || v_payout_event.tipo_evento,
        v_user_id,
        jsonb_build_object(
          'perna_id', v_perna.perna_id,
          'origem', 'reabertura_surebet',
          'projeto_id', v_aposta.projeto_id
        )
      )
